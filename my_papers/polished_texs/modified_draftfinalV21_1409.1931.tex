...
The Einstein-frame volume $\mathcal{V}$ is exponentially large in string units: $\mathcal{V}\sim e^{1/g_s}$ ($g_s$ is the string coupling).
...
\begin{equation}
M_s = \frac{g_s^{1/4} M_P}{\sqrt{4\pi\mathcal{V}}}\,,
\label{Ms}
\end{equation}
the Kaluza-Klein scale is
\begin{equation}
M_{\scriptscriptstyle \rm KK\simeq \frac{M_P}{\sqrt{4\pi}\mathcal{V}^{2/3}}\,,
\label{MKK}
\end{equation}
...
\begin{equation}
m_{3/2} = e^{K/2} |W|= \left(\frac{g_s^2}{2\sqrt{2\pi}}\right) \frac{W_0 M_P}{\mathcal{V}} +\ldots\, ,
\label{m32}
\end{equation}
...
$m_\mathcal{V} \simeq m_{3/2}/\sqrt{\mathcal{V}}$. Hence there is a natural hierarchy of scales $M_s\gg M_{\scriptscriptstyle \rm KK\gg m_{3/2}\gg m_\mathcal{V}$
...
then the volume is of order $\mathcal{V}\simeq 10^{3}-10^{7}$ for $g_s\simeq 0.1$.
...
which suffer from the CMP since $m_\mathcal{V} \simeq 10$ GeV.}
...
Requiring TeV scale soft-terms in an unsequestered setting leads to a volume of order $\mathcal{V}\simeq 10^{14}$ for $W_0\sim 10$,
...
Typically gaugino masses are of order $M_{1/2}\simeq m_{3/2}/\mathcal{V}$,
whereas scalar masses can be as suppressed as the gaugino masses or hierarchically larger by a power $\mathcal{V}^{1/2}$
...
and no CMP for $\mathcal{V}\simeq 10^7$ and $W_0\simeq 50$ which give
...
\texttt{Large$\mathcal{V}$ol} can calculate F-terms and soft-terms generated via both supergravity and anomaly mediation.}
...
Broadly, we find two classes of models: scenarios in which all soft-terms are of order $m_{3/2}/\mathcal{V}$
and scenarios where gaugino masses and A-terms are of this order but scalar masses are of order $m_{3/2}/\mathcal{V}^{1/2}$.
...
of a point-like singularity \cite{Cicoli:2008va}. For these manifolds the volume~$\mathcal{V}$ is of Swiss-cheese type
\begin{equation}
\mathcal{V} = \alpha_b \tau_b^{3/2} - \sum_i \alpha_i \tau_i^{3/2}\,,
\label{eq:vol}
\end{equation}
...
supporting non-perturbative effects, the visible sector cycle $T_{\scriptscriptstyle \rm SM$ and its orientifold image $G$.
...
\begin{equation}
T_b = \tau_b + i \psi_b\,, \quad T_s = \tau_s + i \psi_s\,, \quad T_{\scriptscriptstyle \rm SM = \tau_{\scriptscriptstyle \rm SM + i \psi_{\scriptscriptstyle \rm SM\,, \quad G = b + i c\,,
\label{eq:cycles}
\end{equation}
where $\tau_b$, $\tau_s$ and $\tau_{\scriptscriptstyle \rm SM\textrightarrow 0$ are divisor volumes, the $\psi$'s are axions given by the reduction of $C_4$ on each of the relevant four-cycles,
whereas $b$ and $c$ are respectively the reduction of $B_2$ and $C_2$ on the two-cycle dual to the shrinking one.
The CY volume $\mathcal{V}$ is a function of the K\"ahler moduli which takes the same form as in (\ref{eq:vol}).
...
\begin{equation}
W= W_{\rm flux}(U,S) + A_s(U, S)\, e^{- a_s T_s} +  W_{\scriptscriptstyle \rm dS+W_{\rm matter}\,.
\label{W}
\end{equation}
...
The term $W_{\scriptscriptstyle \rm dS$ involves the contribution from the mechanism used to obtain a dS vacuum (see Sec.~\ref{dsv})
...
\begin{equation}
\label{generalk}
K = - 2 \ln\left(\mathcal{V} + \frac{\hat\xi}{2}\right) - \ln(2s) + \lambda_{\scriptscriptstyle \rm SM \frac{\tau_{\scriptscriptstyle \rm SM^2}{\mathcal{V}} + \lambda_b \frac{b^2}{\mathcal{V}}
+ K_{\scriptscriptstyle \rm dS + K_{\rm cs}(U) + K_{\rm matter}\, ,
\end{equation}
where $\hat\xi\equiv \xi s^{3/2}$, the $\lambda$'s are $\mathcal{O}(1)$ coefficients, $K_{\rm cs}(U)$ is the tree-level K\"ahler potential for complex structure moduli and $K_{\scriptscriptstyle \rm dS$ encodes the dependence on the sector responsible
...
\begin{equation}
\tilde{K}_{\alpha} = \frac{f_\alpha(U,S)}{{{\cal{V^{2/3}} \left(1 - c_s \frac{\hat\xi}{{{\cal{V}
+ \tilde{K}_{\scriptscriptstyle \rm dS + c_{\scriptscriptstyle \rm SM \tau_{\scriptscriptstyle \rm SM^p + c_b b^p\right), \qquad p > 0\,,
\label{mattermetric}
\end{equation}
where we have used $\tilde{K}_{\scriptscriptstyle \rm dS$ to parameterise the dependence on the
...
in order to have a well-behaved metric in the singular limit $b,\tau_{\scriptscriptstyle \rm SM \textrightarrow 0$.
...
\end{eqnarray}
The vanishing D-term condition fixes therefore $\tau_{\scriptscriptstyle \rm SM$ and $b$ in terms of visible sector matter fields.
...
However $\tau_{\scriptscriptstyle \rm SM$ and $b$ would still be fixed in the singular regime since their VEVs would be volume-suppressed \cite{Cicoli:2013cha}.}
Hence the D-term potential (\ref{dtermpot}) vanishes in the vacuum since it is fixed to a supersymmetric minimum at $\xi_1 = \xi_2 = 0$. This corresponds to the singular limit $\tau_{\scriptscriptstyle \rm SM = b = 0$. In turn, the axions $\psi_{\scriptscriptstyle \rm SM$ and $c$ are eaten up by the two $U(1)$ gauge bosons in the process of anomaly cancellation.

\subsubsection{F-term stabilisation}

Analysing the F-term scalar potential in an inverse volume expansion, one finds that the leading contribution scales as $\mathcal{V}^{-2}$.
This is generated by the flux superpotential $W_{\rm flux}$ and is positive semi-definite
\begin{equation}
\label{vddilaton}
V_{\mathcal{O}({\mathcal{V}}^{-2})} = \frac{1}{2 s\mathcal{V}^2} \left.\left[4 s^2 |D_S W_{\rm flux}|^2
...
and non-perturbative corrections to $W$ (\ref{W}) which give rise to $\mathcal{O}(\mathcal{V}^{-3})$ contributions
...
\begin{equation}
\label{v0}
V_{\mathcal{O}({\mathcal{V}}^{-3})} = \frac{1}{2 s} \left[\frac{8}{3}(a_s A_s)^2 \sqrt{\tau_s} \frac{e^{- 2 a_s \tau_s}}{\mathcal{V}}
...
\label{tausms}
\end{equation}
where $f_{\scriptscriptstyle \rm dS$ is a subdominant function of $\epsilon_s$ which depends on the particular mechanism used to obtain a dS vacuum (see Appendix~\ref{App}).
The relation (\ref{tausms}) implies that at the minimum (neglecting $f_{\scriptscriptstyle \rm dS$)
\begin{equation}
...
reads (we neglect $\mathcal{O}(\epsilon_s)$ effects)
\begin{equation}
D_S W \simeq \left.D_S W_{\rm flux}\right|_{\xi = 0} - \frac{3\hat\xi W_0}{4s\mathcal{V}}
\left[1 + \epsilon_s s\partial_s \ln A_s(U,S)\right]\,.
\end{equation}
Since we do not know the functional dependence of $A_s(U,S)$ and since the $W_{\scriptscriptstyle \rm dS$ term in (\ref{W})
...
\begin{equation}
D_U W \simeq \left.D_U W_{\rm flux}\right|_{\xi = 0} - \frac{3\hat\xi W_0}{4\mathcal{V}}
\epsilon_s \partial_u\left[K_{\rm cs}(U)+  \ln A_s(U,S)\right]\,,
\end{equation}
and so we can parameterise this shift by $\omega_{U_i}(U,S)$ as
\begin{equation}
\label{csmin}
D_{U_i} W = - \frac{3 \omega_{U_i} (U,S)}{4} \frac{\hat\xi W_0}{s\mathcal{V}}
\qquad\Rightarrow\qquad D_{U_i} W =\frac{\omega_{U_i} (U,S)}{\omega_S(U,S)} D_S W\sim \mathcal{O}(\mathcal{V}^{-1})\,,
\end{equation}
where both $\omega_S$ and $\omega_{U_i}$ are expected to be $\mathcal{O}(1)$ functions of $S$ and $U$. Note that both functions $\omega_{S,U_i}$ depend also on the dS mechanism.
...
(focusing for simplicity on a single matter field $\phi_{\scriptscriptstyle \rm dS$ with K\"ahler metric $K_{\scriptscriptstyle \rm dS= s^{-1}|\phi_{\scriptscriptstyle \rm dS|^2$ \cite{Conlon:2006tj, Aparicio:2008wh} and $U(1)$-charge $q_\phi$)
\begin{equation}
\label{dmatter}
...
\label{Vtot}
\end{equation}
where $m_{3/2}$ is the gravitino mass as in eq.~(\ref{m32}) and $V_{\mathcal{O}(\mathcal{V}^{-3})}$ is given in (\ref{v0}).
If the two $U(1)$-charges $q_\phi$ and $q_b$ have the same sign,
$\phi_{\scriptscriptstyle \rm dS$ develops a non-vanishing VEV
\begin{equation}
\label{minphi}
...
\label{VD0ds1}
\end{equation}
Following \cite{Cicoli:2012vw}, we can minimise the total scalar potential (\ref{VTOT}) with respect to $\tau_s$ and $\mathcal{V}$,
...
\begin{equation}
\left[\ln\left(\frac{\langle\mathcal{V}\rangle}{W_0}\right)\right]^{3/2}= \delta^3\, \langle\mathcal{V}\rangle \sim 5\cdot 10^{-6}\, \langle\mathcal{V}\rangle\qquad\Leftrightarrow\qquad
...
\label{CCconddS1}
\end{equation}
For natural $\mathcal{O}(1)$ values of all underlying parameters, this relation gives a minimum for $\mathcal{V}$ at order $10^6 - 10^7$ (see~\cite{Cicoli:2012vw}).
...
\end{equation}
with $\tau_{\scriptscriptstyle \rm dS={\rm Re}(T_{\scriptscriptstyle \rm dS)$. This blow-up mode can be fixed in the singular regime by minimising the hidden sector D-term potential
...
\label{Vtot1}
\end{equation}
where the second term comes from the new superpotential (\ref{Wds2}) and $V_{\mathcal{O}(\mathcal{V}^{-3})}$ is given in~(\ref{v0}).
Minimisation with respect to $\tau_{\scriptscriptstyle \rm dS$ gives
\begin{equation}
...
such that $\langle\sum_i q_{{\rm hid},i} |\phi_{{\rm hid},i}|^2\rangle = 0$ but $A_{\scriptscriptstyle \rm dS\neq 0$,\footnote{In order to make $W_{\scriptscriptstyle \rm dS$ gauge invariant,
$A_{\scriptscriptstyle \rm dS$ has to depend on the $\phi_{{\rm h},i}$ which can develop non-zero VEVs for appropriate hidden field F-term contributions, giving $A_{\scriptscriptstyle \rm dS\neq 0$ with $\tau_{\scriptscriptstyle \rm dS$ in the singular regime \cite{Cicoli:2012fh}.} and substituting the VEV (\ref{tdsVEV}) in (\ref{Vtot1}) we obtain at leading order
\begin{equation}
...
at $\mathcal{O}(\mathcal{V}^{-3})$ requires to tune 3-form fluxes such that
\begin{equation}
...
if we perform an expansion in $\mathcal{V}^{-1}$ and $\epsilon_s$. We also factor out the gravitino mass which is given by the following expression
\begin{equation}
m_{3/2}=e^{K/2} |W|= \left(\frac{g_s^2 M_P}{2\sqrt{2\pi}}\right)\frac{W_0}{\mathcal{V}} \left[ 1-\frac{\hat{\xi}}{2\mathcal{V}}
\left(1+3 y_{\scriptscriptstyle \rm dS \epsilon_s+\mathcal{O}(\epsilon_s^2)\right)+\mathcal{O}\left(\frac{1}{\mathcal{V}^2}\right)\right]\,,
\label{expandm32}
\end{equation}
where $y_{\scriptscriptstyle \rm dS =1$ for the dS case 1 of Sec.~\ref{matteruplift} while $y_{\scriptscriptstyle \rm dS = 1+\frac{\sqrt{2} a_s^{3/4}}{\kappa_{\scriptscriptstyle \rm dS a_{\scriptscriptstyle \rm dS}$ for the dS case 2 of Sec.~\ref{dilatonuplift}. The leading order F-terms for $T_b$ and $T_s$ turn out to be
...
\label{FTb}
\end{equation}
where $x_{\scriptscriptstyle \rm dS = - 45/16$ for the dS case 1 of Sec.~\ref{matteruplift} while $x_{\scriptscriptstyle \rm dS \sim \mathcal{O}(1/\mathcal{V})$ for the dS case 2 of Sec.~\ref{dilatonuplift}
...
are unknown $\mathcal{O}(1)$ functions of $U$ and $S$.
Additional non-zero F-terms are associated to fields responsible to achieve a dS solution.
For the dS case 1 of Sec.~\ref{matteruplift} there is an F-term associated to $\phi_{\scriptscriptstyle \rm dS$
\begin{equation}
\frac{F^{\phi_{\scriptscriptstyle \rm dS}}{\phi_{\scriptscriptstyle \rm dS} \simeq m_{3/2}\,,
\end{equation}
with $\phi_{\scriptscriptstyle \rm dS$ given in (\ref{CCconddS1}) (up to an irrelevant phase). On the other hand,
in the dS case 2 of Sec.~\ref{dilatonuplift} the blow-up mode $T_{\scriptscriptstyle \rm dS$ has a non-vanishing F-term
...
to the overall volume of the compactification. Thus for (\ref{yuk}) to yield a result independent of $\mathcal{V}$
one needs at leading order in an inverse volume expansion (for $\tau_{\scriptscriptstyle \rm SM = b= C^\alpha= 0$)
\begin{equation}
\label{ul}
\tilde{K}_\alpha = h_\alpha(U,S)\,e^{K/3} \simeq  \frac{h_\alpha(U,S)\,e^{K_{\rm cs}/3}}{(2s)^{1/3}\mathcal{V}^{2/3}}
\left(1- \frac{\hat\xi}{3\mathcal{V}}  + \frac 13 K_{\scriptscriptstyle \rm dS\right)\,,
\end{equation}
where $h_\alpha(U,S)$ is an unknown function of $U$ and $S$ and in the approximation we focus on the first subleading order corrections, e.g.~neglecting higher order corrections of $\mathcal{O}\left(1/\mathcal{V}^{8/3}\right)$. Note
that this result has the same volume scaling of our formula for the matter metric (\ref{mattermetric}) which for $\tau_{\scriptscriptstyle \rm SM = b= 0$ reduces to
\begin{equation}
...
beyond leading order in a $\mathcal{V}^{-1}$ expansion. There is no reason
...
\end{itemize}
If we parameterise $\tilde{K}_{\scriptscriptstyle \rm dS$ as $\tilde{K}_{\scriptscriptstyle \rm dS  =  c_{\scriptscriptstyle \rm dS K_{\scriptscriptstyle \rm dS$,
...
\label{fdef}
\end{equation}
Subleading deviations from the approximation in~(\ref{ul}) can be accounted for by small changes in $c_s$ and $c_{\scriptscriptstyle \rm dS$ at the appropriate subleading order.
...
leading order in $\mathcal{V}^{-1}$ and $\epsilon_s$.
...
\end{equation}
where $f_a=\delta_a S+ \kappa_a \,T_{\scriptscriptstyle \rm SM$ is the gauge kinetic function as in~\eqref{eq:gaugekineticf}. As $F^{T_{\scriptscriptstyle \rm SM}=0$, we obtain universal gaugino masses,
...
\simeq \frac{5\left(c_s-\frac 13\right)}{\omega'_S}\, m_{3/2}M_{1/2}\sim \mathcal{O}\left(m_{3/2}^2\frac{\left(\ln\mathcal{V}\right)^{3/2}}{\mathcal{V}}\right)\,.
\label{localscalar}
\end{equation}
...
can arise from $F^S$ and $F^{U_i}$ but they are volume suppressed since they would give contributions of order $m_{3/2}^2/\mathcal{V}^2$. If $c_s>1/3,$ the scalar masses are non-tachyonic.
...
at $\mathcal{O}(\mathcal{V}^{-3})$. Using (\ref{ul}) (the defining property of ultra-local models)
...
Recalling that $V_0 = V_{F,0} + V_{D,0}$ and setting the cosmological constant to zero (in the dS constructions of Sec.~\ref{dsv} we showed how to cancel $V_0$ at $\mathcal{O}(\mathcal{V}^{-3})$ but this can in principle be done at any order in the $\mathcal{V}^{-1}$ expansion), $V_{F,0}$ can be traded for $V_{D,0}$,
...
from the F-terms of the dilaton and the complex structure moduli at $\mathcal{O}(\mathcal{V}^{-2})$. Using (\ref{ds}),
...
+ \beta^{U_i}\beta^{\overline{U}_j}\partial_{u_i} \partial_{u_j}\right) \ln h_\alpha(U,S)\sim \mathcal{O}\left(M_{1/2}^2\right)\,,
\label{ulsm3}
\end{equation}
...
As explained in Sec.~\ref{shrink}, the VEV of the D-term potential associated to visible sector $U(1)$s is vanishing in the absence of tachyonic scalars.\footnote{Even in the presence of tachyonic scalars, the contribution to scalar masses from visible sector D-terms turns out to be a negligible effect since visible matter fields, $\tau_{\scriptscriptstyle \rm SM$ and $b$ would still be stabilised at zero at leading order.}
...
As can be seen from (\ref{VD0ds1}), $V_{D,0}$ scales as $\mathcal{V}^{-10/3}$ which is subdominant with respect to the first term in (\ref{genscalD}) that gives
\begin{equation}
...
\label{sc1}
\end{equation}
once we impose the condition (\ref{CCconddS1}) to have a vanishing cosmological constant at $\mathcal{O}(\mathcal{V}^{-3})$.
...
\label{Vds22}
\end{equation}
As can be seen from (\ref{VD0ds2}), $V_{D,0}$ scales as $\mathcal{V}^{-4}$. Hence in the local limit the D-term contribution
is subleading with respect to the F-term one given in (\ref{localscalar}) which scales as $\mathcal{V}^{-3}$.
In the ultra-local limit the F-term contribution to scalar masses is given by (\ref{ulsm2}).
Adding $-V_{F,0}/3= V_{D,0}/3$ to (\ref{Vds22}) we find that the total D-term contribution to scalar masses vanishes in the ultra-local limit once we impose $c_{\scriptscriptstyle \rm dS=1/3$ as in (\ref{fdef}) since
\begin{equation}
...
Their expression is given in (\ref{ulsm3}) and scales as $\mathcal{V}^{-4}$.\\
...
it is given by the F-term contribution (\ref{localscalar}) that scales as $\mathcal{V}^{-3}$. Scalar masses are non-tachyonic if $c_s>1/3$ and universal.
On the other hand, the result for the ultra-local limit depends on the dS mechanism.
In the dS case 1 of Sec.~\ref{matteruplift}, scalar masses get generated by the D-term contribution (\ref{sc1}) which has again an overall $\mathcal{V}^{-3}$ scaling but with an $\epsilon_s$ suppression with respect to the local case. Scalar masses turn out to be non-tachyonic and universal.
...
Let us present the expression for $A_{\alpha\beta\gamma}$ at leading order in $\mathcal{V}^{-1}$ and $\epsilon_s$.
...
- s \partial_{s,u} \ln \left(\frac{Y_{\alpha \beta \gamma}}{f_\alpha f_\beta f_\gamma} \right) \right] M_{1/2}\sim\mathcal{O}\left(M_{1/2}\right)\,,
\end{equation}
...
Let us compute the leading expressions (in an expansion in $\mathcal{V}^{-1}$ and $\epsilon_s$) for both $\hat\mu$ and $B\hat\mu$
...
\label{mu11}
\end{equation}
where again there is a cancellation at $\mathcal{O}(\mathcal{V}^{-1})$ between the term proportional to $m_{3/2}$ in (\ref{muK})
...
contribute to the effective action. If $T$ is a bulk cycle, the coupling (\ref{Wnpmu}) is always generated but in our case it would be negligible since this effect would be proportional to $e^{-\mathcal{V}^{2/3}}$. On the other hand, if $T$ is the blow-up of a local singularity, the couplings (\ref{Wnpmu}) and (\ref{Wnpmu2})
get generated only if this divisor shares a homologous two-cycle with the blow-up mode $T_{\scriptscriptstyle \rm SM$ of the MSSM singularity.
This condition is not satisfied if either $T$ or $T_{\scriptscriptstyle \rm SM$ is a very simple divisor like a dP$_0$ which has been used in the explicit global models of \cite{Cicoli:2012vw} and \cite{Cicoli:2013cha}.
...
\begin{equation}
\hat{\mu} \simeq \frac{c_\mu(U,S)}{\mathcal{V}^{n+\frac 13}}
\qquad\text{and}\qquad
B \hat{\mu} \simeq \frac{c_B(U,S)}{\mathcal{V}^{n+ \frac 43}}\,,
\label{mumag}
\end{equation}
...
in terms of $\tau_{\scriptscriptstyle \rm SM$: $|\Phi|^2 \propto \tau_{\scriptscriptstyle \rm SM/\mathcal{V}$,
and so the couplings in (\ref{KWphi}) would give rise to effective $\mu$ and $Z$-terms which depend only on closed string moduli
\begin{equation}
Z_{\rm eff}\propto \frac{1}{\Lambda^n}\left(\frac{\tau_{\scriptscriptstyle \rm SM}{\mathcal{V}}\right)^{n/2}\,,
\quad \quad\mu_{\rm eff}\propto \frac{1}{\Lambda^{n-1}}\left(\frac{\tau_{\scriptscriptstyle \rm SM}{\mathcal{V}}\right)^{n/2}\,.
\label{Eff}
\end{equation}
...
The result will depend on the VEV and the F-term of $T_{\scriptscriptstyle \rm SM$. As discussed in \cite{Cicoli:2013cha},
$\Phi$ needs to receive tachyonic contribution from soft terms in order for $T_{\scriptscriptstyle \rm SM$ to develop a non-zero VEV.
If this condition is satisfied, $\tau_{\scriptscriptstyle \rm SM \sim \mathcal{V}^{-1}$ implying $F^{T_{\scriptscriptstyle \rm SM}\sim \mathcal{V}^{-2}$ for the local case
...
However in both cases the VEV of $\tau_{\scriptscriptstyle \rm SM$ is smaller than unity, and so we are still consistently in the singular regime.
...
depend on $\Lambda$ is for $n=1$. In this situation $\hat\mu$ would scale as $\mathcal{V}^{-4/3}$ in the local case and as $\mathcal{V}^{-7/3}$ in the ultra-local case.
...
(or the cut-off scale) which is the Kaluza-Klein scale $M_{\scriptscriptstyle \rm KK\sim M_P/\mathcal{V}^{2/3}$.
...
\begin{equation}
\delta m=\alpha_{\scriptscriptstyle \rm loop \frac{M_{\scriptscriptstyle \rm KK m_{3/2}}{M_P}\sim \alpha_{\scriptscriptstyle \rm loop\frac{W_0}{\mathcal{V}^{5/3}}\ll \alpha_{\scriptscriptstyle \rm loop m_{3/2}\,,
\end{equation}
with $\alpha_{\scriptscriptstyle \rm loop\sim g^2/(16\pi^2)$ a loop factor. Note that the ratio $\delta m/m\sim \alpha_{\scriptscriptstyle \rm loop \mathcal{V}^{-1/6}$ is very small
...
Therefore masses as small as $M_{\rm soft}\sim W_0/\mathcal{V}^2$ are still stable under standard loop corrections
...
$\delta m \sim \alpha_{\scriptscriptstyle \rm loop m_{3/2}^2/M_P\sim \alpha_{\scriptscriptstyle \rm loop M_P/\mathcal{V}^2$. More recently, explicit calculations for gravitino loop contributions to gaugino masses was performed in \cite{Lee:2013aia}. The diagrams are quadratically divergent and proportional to the gravitino mass:
\begin{equation}
...
In string theory we expect that $\Lambda\leq M_s\sim M_P/\mathcal{V}^{1/2}$ which then corrects the gaugino masses to order
$\delta M_{1/2}\leq \alpha_{\scriptscriptstyle \rm loop M_P/\mathcal{V}^2$ which is smaller than the sequestered gaugino masses $M_{1/2}\sim M_P/\mathcal{V}^2$.
...
For $g\simeq 0.1$ and $\mathcal{V} \simeq 5\cdot 10^6$ (the value needed to get $M_{1/2}^{\rm new}$ approximately around the TeV-scale), $c$ is roughly of order $c'\times10^{-3}.$ Depending on the exact value of $c',$ which is beyond the scope of this analysis, we can achieve competing contributions from moduli mediation and anomaly mediation.
...
For $M_{\rm soft}^2 \sim m_{3/2}^2/\mathcal{V}^n$ the strength of CP and flavour violation induced by A-terms would be
\begin{equation}
\label{flav}
...
$\mathcal{V} < 10^5$. This gives a slight tension with our results but there can be several ways around this
