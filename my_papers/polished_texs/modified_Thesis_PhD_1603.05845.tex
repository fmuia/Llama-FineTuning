...
\def\nonumber{\nonumber} 
\def\partial {\partial} 
\defCalabi--Yau {Calabi--Yau} 
\defClifford(d,\,d) {Clifford(d,\,d)} 
\defClifford(d) {Clifford(d)} 
\defClifford(6,6) {Clifford(6,6)} 
\def\hat Q{\hat Q} 
\def{\boldsymbol 0 {\nonumber} 
\def\partial{\partial} 
\def\Omega{\Omega} 
\def\begin{align{\begin{align}r \jmath}
\def\minicent#1#2{
  \begin{minipage}{#1 cm}
    \begin{center}
     #2 
...
{\bf 3}&\minicent{3.2}{\vspace{.2cm} $\overline{\mathcal{W}}_5=2\mathcal{W}_4= \qquad \qquad=\mp 2iH_3^{(\overline 3)}=\overline{\partial}\phi\,$;\\
$\overline{\partial} A=\overline{\partial} a=0\,$.\vspace{.2cm}}&\multicolumn{2}{c|}{
...
{\bf 3}& {\minicent{3.5}{\vspace{0.3cm}$\overline{\mathcal{W}}_5=2\mathcal{W}_4= \qquad=\mp 2iH_3^{(\overline 3)}= 2 \overline\partial\phi\,$;\\\vspace{0.3cm} $\overline\partial A=\overline\partial a=0\,$.}}
 & 
...
In order to get the four-dimensional spectrum of the compactification, it is necessary to perform a Kaluza-Klein (KK) reduction of the ten-dimensional theory on a compact internal manifold $\chi$. This procedure typically gives rise to a tower of states, called \textit{Kaluza-Klein (KK) states} whose masses scale as\footnote{In this rough estimate we assume that the compactification manifold is isotropic.} $m_n \sim n M_s/R$, where $R$ is the radius of the compact space. $R$ can be rewritten in terms of the volume $\mathcal{V}$ of the compact space as $R = \ell_s \mathcal{V}^{1/6}$. As we have have observed in Sec.~\ref{sec:StringTheory} $M_s \sim M_{\rm P} \mathcal{V}^{-1/2}$, so that the KK states masses are given by
\begin{align}
\label{eq:KKTower}
M_n \sim \frac{n M_{\rm P}}{\mathcal{V}^{2/3}}\,.
\end{align}
...
\begin{align}
\label{eq:KKScale}
E \ll M_{KK} \simeq \frac{M_{\rm P}}{\mathcal{V}^{2/3}}\,,
\end{align}
namely we consider only massless KK fields, which correspond to harmonic forms of the compact internal space~\cite{Candelas:1990pi}. Eq. \eqref{eq:KKScale} is called \textit{KK approximation}. In order to trust the supergravity approximation we require $\mathcal{V} \gg 1$, then there is a hierarchy between the string scale and the KK scale
\begin{align}
...
\end{align}
Let us notice that with these conventions the volume $\mathcal{V}$ of the compact space (measured in units of string length $\ell_s = 2 \pi \sqrt{\alpha'}$) is given by
\begin{align}
\label{eq:VolumeFormal}
\mathcal{V} = \frac{\mathcal{K}}{6} \,,
\end{align}
...
K_0 = - 2 \text{ln} \mathcal{V} - \text{ln} \left(-i \left(\tau - \overline{\tau}\right)\right) - \text{ln} \left(-i \int_\chi \Omega \wedge \overline{\Omega} \right)\,,
\end{align}
...
\mathcal{V} \equiv \frac{\mathcal{K}\left(t^i(T_i + \overline{T}_i, \tau - \overline{\tau}, G^\kappa - \overline{G}^\kappa)\right)}{6} \,,
\end{align}
where $\mathcal{V}$ is the volume of the CY space measured in the Einstein frame and in string units $\ell_s = 2 \pi \sqrt{\alpha'}$. Even if it is not always possible to invert eq. \eqref{eq:FieldRedefinition} in order to get $t^i(\tau, T, G)$, in eq. \eqref{eq:KahlerPotential} we should regard $\mathcal{V}$ as a function of $(\tau - \overline{\tau}, T_i + \overline{T}_i, G^{\kappa} - \overline{G}^\kappa)$, where these combinations of the fields $\tau, T_i, G^{\kappa}$ are due to the shift-symmetry of axions of eq. \eqref{eq:ShiftSymmetry}.\\
...
m_{U^a} \sim m_S \simeq \int_{\Sigma_3} F_3 \simeq \int_{\Sigma_3} H_3 \sim \frac{\alpha'}{R^3} = \frac{M_{\rm P}}{\mathcal{V}} \,,
\end{align}
...
K \supset - 2 \log \left(\mathcal{V} + \frac{\xi \left(S + \overline{S}\right)^{3/2}}{2}\right) \,,
\end{equation}
where
\begin{align}
\label{eq:XiHat}
...
\delta K^{\rm KK}_{(g_s)} = - \frac{1}{128 \pi^4} \sum_{i=1}^{h^{(1,1)}} \frac{\mathcal{C}_i^{\rm KK}(U, \overline{U}) \left(a_{il} t^l\right)}{\text{Re}(S) \mathcal{V}} \,,
\end{align}
...
\delta K^{\rm W}_{(g_s)} = - \frac{1}{128 \pi^4} \sum_{i=1}^{h^{(1,1)}} \frac{\mathcal{C}_i^{\rm W}(U, \overline{U})}{\left(a_{il} t^l\right) \mathcal{V}} \,,
\end{align}
...
\begin{align}
\label{eq:VolumeLVS}
\mathcal{V} = \alpha_b \tau_b^{3/2} - \sum_{i=2}^{h^{(1,1)}_+} \alpha_i \tau_i^{3/2}\,,
\end{align}
...
K = - 2 \log\left(\mathcal{V} + \frac{\hat{\xi}}{2}\right) - \log\left(S + \overline{S}\right) + \log\left(i \int_\chi \Omega \wedge \overline{\Omega}\right) \,,
\end{align}
...
\begin{align}
e^{- a_s \tau_s} = \frac{3 \sqrt{\tau_s} W_0}{4 a_s A_s \mathcal{V}} \frac{\left(1-4\epsilon_s\right)}{\left(1-\epsilon_s\right)}\qquad\text{with}\qquad \epsilon_s \equiv\frac{1}{4 a_s \tau_s}\sim\mathcal{O}\left(\frac{1}{\ln\mathcal{V}}\right)\ll 1\,.
...
In terms of the volume $\mathcal{V}$, the string scale is (see Appendix~A of~\cite{Conlon:2005ki} for the derivation of the exact pre-factors)
\begin{align}
...
\begin{align}
M_{\scriptscriptstyle \rm KK\simeq \frac{M_{\rm P}}{\sqrt{4\pi}\mathcal{V}^{2/3}}\,,
\label{MKK}
\end{align}
...
\tilde{K}_\alpha \approx \frac{1}{\mathcal{V}^{2/3}} \,.
\end{align}
...
\begin{align}
K \supset \lambda_{\rm vis} \frac{\left(T_{\rm vis} + \overline{T}_{\rm vis} + q_{\rm vis} V_{\rm vis}\right)^2}{\mathcal{V}} \,,
\end{align}
...
\mathcal{V} = \frac{1}{9} \sqrt{\frac{2}{3}} \left(\tau_b^{3/2} - \sqrt{3} \left(\tau_{q_1}^{3/2} + \tau_{q_2}^{3/2} + \tau_s^{3/2}\right)\right)\,,
\end{align}
where
\begin{align}
...
K = - 2 \text{log} \left(\mathcal{V} + \frac{\hat{\xi}}{2}\right) - \text{log} \left(S + \overline{S}\right) + \text{log} \left(i \int_\chi \Omega \wedge \overline{\Omega}\right) + \nonumber \\
+ \frac{\left(T_{\rm SM} + \overline{T}_{\rm SM} + q_{\rm SM} V_{\rm SM}\right)^2}{\mathcal{V}} + \frac{\left(G + \overline{G} + q_G V_G\right)^2}{\mathcal{V}} + \tilde{K_\alpha} C^\alpha \overline{C}^{\overline{\alpha}}\,,
\end{align}
...
\begin{align}
W= W_0(U,S) + A_s(U, S)\, e^{- a_s T_s} +  W_{\scriptscriptstyle \rm dS+W_{\rm matter}\,.
\label{eq:FullSuperpotential}
\end{align}
As explained in Sec.~\ref{sssec:CorrectionsToTreeLevel}, the pre-factor $A_s(U,S)$ depends on both complex structure moduli $U$ and the dilaton $S$, and it is an $\mathcal{O}(1)$ function.\footnote{The dependence on $S$ and $U$-moduli is structurally different, i.e.~the dependence on the dilaton is generated when including the back-reaction of sources and warping on the geometry~\cite{Baumann:2006th}.} The term $W_{\scriptscriptstyle \rm dS$ involves the contribution from the mechanism used to obtain a dS vacuum (see Sec.~\ref{ssec:deSitterScenarios}) while $W_{\rm matter}$ is the visible sector superpotential
\begin{align}
...
\begin{align}
\label{generalk}
K = - 2 \ln\left(\mathcal{V} + \frac{\hat\xi}{2}\right) - \ln(2s) + \lambda_{\scriptscriptstyle \rm SM \frac{\tau_{\scriptscriptstyle \rm SM^2}{\mathcal{V}} + \lambda_b \frac{b^2}{\mathcal{V}} + K_{\scriptscriptstyle \rm dS + K_{\rm cs}(U) + K_{\rm matter}\, ,
\end{align}
where $\hat\xi\equiv \xi s^{3/2}$, the $\lambda$'s are $\mathcal{O}(1)$ coefficients, $K_{\rm cs}(U)$ is the tree-level K\"ahler potential for complex structure moduli and $K_{\scriptscriptstyle \rm dS$ encodes the dependence on the sector responsible for obtaining a dS vacuum (see Sec.~\ref{ssec:deSitterScenarios}). The matter K\"ahler potential $K_{\rm matter}$ is taken to be
\begin{align}
...
+ \tilde{K}_{\scriptscriptstyle \rm dS + c_{\scriptscriptstyle \rm SM \tau_{\scriptscriptstyle \rm SM^p + c_b b^p\right), \qquad p > 0\,,
\label{mattermetric}
\end{align}
where we have used $\tilde{K}_{\scriptscriptstyle \rm dS$ to parametrize the dependence on the dS mechanism (details will be given in Sec.~\ref{soft}). The $c$'s are taken as constants for simplicity while $p$ is taken to be positive in order to have a well-behaved metric in the singular limit $b,\tau_{\scriptscriptstyle \rm SM \textrightarrow 0$. As they can in principle depend on $U$ and $S$, we comment in due course on the influence on the soft-terms of such a dependence. The appearance of the higgs bilinear and its potential parametrization are discussed in Sec.~\ref{sssec:SoftTerms} when we analyze the $\mu$-term in this scenario. In general the functions $f_\alpha(U,S)$ could be non-universal. Such non-universality can have interesting phenomenological implications (e.g.~mass hierarchies among families of sfermion masses needed for a realization of natural SUSY). As we are interested in soft-terms arising for D-branes at singularities, we take the gauge kinetic function to be
\begin{align}
f_a=\delta_a S+ \kappa_a \,T_{\scriptscriptstyle \rm SM\, ,
...
\begin{align}
\xi_1 &= - \frac{q_1}{4\pi} \frac{\partial K}{\partial T_{\scriptscriptstyle \rm SM} = - \frac{q_1  \lambda_{\scriptscriptstyle \rm SM}{4\pi} \frac{\tau_{\scriptscriptstyle \rm SM}{\mathcal{V}}\,, \label{xiSM} \\
&\xi_2 = - \frac{q_2}{4\pi} \frac{\partial K}{\partial G} = - \frac{q_2 \lambda_b}{4\pi} \frac{b}{\mathcal{V}}\,.
\end{align}
The vanishing D-terms condition fixes therefore $a_{\scriptscriptstyle \rm SM$ and $b$ in terms of visible sector matter fields.
The remaining flat directions are fixed by sub-leading F-terms contributions which give vanishing VEVs to the $C^\alpha$ if they develop non-tachyonic soft masses from SUSY-breaking~\cite{Cicoli:2012vw}.\footnote{If the soft scalar masses of some $C^\alpha$ are tachyonic, they develop non-zero VEVs (which could be phenomenologically allowed for some Standard Model singlets) that, in turn, induce non-zero FI-terms~\cite{Cicoli:2013cha}. However $\tau_{\scriptscriptstyle \rm SM$ and $b$ would still be fixed in the singular regime since their VEVs would be volume-suppressed~\cite{Cicoli:2013cha}.} Hence the D-terms potential \eqref{dtermpot} vanishes in the vacuum since it is fixed to a supersymmetric minimum at $\xi_1 = \xi_2 = 0$. This corresponds to the singular limit $\tau_{\scriptscriptstyle \rm SM = b = 0$. In turn, the axions $a_{\scriptscriptstyle \rm SM$ and $c$ are eaten up by the two $U(1)$ gauge bosons in the process of anomaly cancellation.
...
\begin{align}
\tau_s^{3/2} = \frac{\hat\xi}{2} \left[1 + f_{\scriptscriptstyle \rm dS(\epsilon_s) \right]\,,
\label{eq:TausMinimumCorrected}
\end{align}
where $f_{\scriptscriptstyle \rm dS$ is a sub-dominant function of $\epsilon_s = \frac{1}{4 a_s \tau_s}$ which depends on the particular mechanism used to obtain a dS vacuum, as reported below. The relation \eqref{eq:TausMinimumCorrected} implies that at the minimum (neglecting $f_{\scriptscriptstyle \rm dS$)
\begin{align}
\hat\xi\simeq \frac{1}{4\left(a_s \epsilon_s\right)^{3/2}}\sim\mathcal{O}\left[\left(\ln\mathcal{V}\right)^{3/2}\right] \gg 1\,.
\label{tausms2}
\end{align}
...
Let us try to estimate the shift of $S$ and $U$ from their supersymmetric minimum (\ref{susyminim}) because of $\alpha'$ and non-perturbative effects. The K\"ahler covariant derivative of the total superpotential evaluated at the minimum (\ref{eq:TaubMinimum}) and (\ref{eq:TausMinimumCorrected}) reads (we neglect $\mathcal{O}(\epsilon_s)$ effects)
\begin{align}
D_S W \simeq \left.D_S W_0\right|_{\xi = 0} - \frac{3\hat\xi W_0}{4s\mathcal{V}}
\left[1 + \epsilon_s s\partial_s \ln A_s(U,S)\right]\,.
\end{align}
Since we do not know the functional dependence of $A_s(U,S)$ and since the $W_{\scriptscriptstyle \rm dS$ term in eq. \eqref{eq:FullSuperpotential} can also potentially shift the dilaton minimum, it is not possible for us to compute this shift explicitly. We will parametrize it by using the parameter $\omega_S(U,S)$ defined as
\begin{align}
\label{dilmin}
...
\begin{align}
\label{csmin}
D_{U^a} W = - \frac{3 \omega_{U^a} (U,S)}{4} \frac{\hat\xi W_0}{s\mathcal{V}} \quad\Rightarrow\quad D_{U^a} W =\frac{\omega_{U^a} (U,S)}{\omega_S(U,S)} D_S W\sim \mathcal{O}(\mathcal{V}^{-1})\,,
\end{align}
where both $\omega_S$ and $\omega_{U^a}$ are expected to be $\mathcal{O}(1)$ functions of $S$ and $U$. Note that both functions $\omega_{S/U^a}$ depend also on the dS mechanism. These corrections give rise to $\mathcal{O}\left(\mathcal{V}^{-4}\right)$ contributions to the scalar potential, and then as already mentioned the EFT is self-consistent. They generate a non-vanishing IASD and $G_3^{(0,3)}$ components of the three-form flux $G_3$, which can be seen as small perturbations around the supersymmetric background.
...
generating a moduli-dependent FI-term. The D-terms potential becomes (focusing for simplicity on a single matter field $\phi_{\scriptscriptstyle \rm dS$ with K\"ahler metric $K_{\scriptscriptstyle \rm dS= s^{-1}|\phi_{\scriptscriptstyle \rm dS|^2$~\cite{Conlon:2006tj, Aparicio:2008wh} and $U(1)$-charge $q_\phi$)
\begin{align}
\label{dmatter}
V_D = \frac{1}{2{\rm Re}(f_b)} \left(\frac{q_\phi}{s} |\phi_{\scriptscriptstyle \rm dS|^2 - \xi_b\right)^2\,,
\end{align}
...
\begin{align}
V_{\rm tot} = V_D + V_F =  \frac{1}{2\mathcal{V}^{2/3}} \left(\frac{q_\phi}{s}  |\phi_{\scriptscriptstyle \rm dS|^2 - \frac{3 q_b}{8\pi\mathcal{V}^{2/3}}\right)^2
+ \frac{1}{s} m_{3/2}^2 |\phi_{\scriptscriptstyle \rm dS|^2 + V_{\mathcal{O}(\mathcal{V}^{-3})}\,,
\label{Vtot}
\end{align}
where $m_{3/2}$ is the gravitino mass as in eq.~(\ref{m32}) and $V_{\mathcal{O}(\mathcal{V}^{-3})}$ is given in (\ref{eq:LVSScalarPotential}). If the two $U(1)$-charges $q_\phi$ and $q_b$ have the same sign, $\phi_{\scriptscriptstyle \rm dS$ develops a non-vanishing VEV
\begin{align}
\label{minphi}
\frac{q_\phi}{s} |\phi_{\scriptscriptstyle \rm dS|^2 = \xi_b - \frac{m_{3/2}^2 \mathcal{V}^{2/3}}{q_\phi}\,.
\end{align}
Substituting this VEV in (\ref{Vtot}) we obtain
\begin{align}
V_{\rm tot} = V_{D,0}+\frac{3 q_b}{16\pi q_\phi} \frac{W_0^2}{s  \mathcal{V}^{8/3}}+V_{\mathcal{O}(\mathcal{V}^{-3})}\,,
\label{VTOT}
\end{align}
...
\begin{align}
V_{D,0} = \frac{m_{3/2}^4 \mathcal{V}^{2/3}}{2 q_\phi^2}\sim \mathcal{O}\left(\mathcal{V}^{-10/3}\right)\,.
\label{VD0ds1}
\end{align}
...
\end{align}
A cancellation of the vacuum energy at $\mathcal{O}(\mathcal{V}^{-3})$ requires therefore to tune $W_0$ so that
...
\begin{align}
W_{\scriptscriptstyle \rm dS = A_{\scriptscriptstyle \rm dS(U, S) \,e^{-a_{\scriptscriptstyle \rm dS (S + \kappa_{\scriptscriptstyle \rm dS T_{\scriptscriptstyle \rm dS)}\,.
\label{Wds2}
\end{align}
Because of the presence of an additional K\"ahler modulus, the K\"ahler potential (\ref{generalk}) has to be supplemented with
\begin{align}
\label{dk2}
K_{\scriptscriptstyle \rm dS =  \lambda_{\scriptscriptstyle \rm dS\frac{\tau_{\scriptscriptstyle \rm dS^2}{\mathcal{V}}\,,
\end{align}
with $\tau_{\scriptscriptstyle \rm dS={\rm Re}(T_{\scriptscriptstyle \rm dS)$. This blow-up mode can be fixed in the singular regime by minimizing the hidden sector D-terms potential (focusing for simplicity on canonically normalized hidden fields $\phi_{{\rm h},i}$ with charges $q_{{\rm h},i}$ under an anomalous $U(1)$)
\begin{align}
\label{DtermPot1}
V_D = \frac{1}{2{\rm Re}(f_{\rm h})} \left(\sum_i q_{{\rm h},i} |\phi_{{\rm h},i}|^2 - \xi_{\rm h}\right)^2\,,
\end{align}
where $f_{\rm h} = S$ (neglecting $T_{\scriptscriptstyle \rm dS$-dependent corrections) and the FI-term is given by
($q_{\scriptscriptstyle \rm dS$ is the $U(1)$-charge of $T_{\scriptscriptstyle \rm dS$ and from now on we set $\lambda_{\scriptscriptstyle \rm dS=1$ for simplicity)
\begin{align}
\xi_{\rm h} = - \frac{q_{\scriptscriptstyle \rm dS}{4\pi} \frac{\partial K}{\partial T_{\scriptscriptstyle \rm dS} = - \frac{q_{\scriptscriptstyle \rm dS}{4\pi} \frac{\tau_{\scriptscriptstyle \rm dS}{\mathcal{V}}\,.
\end{align}
In fact, the total scalar potential takes the leading order form (after fixing the axionic phase of $T_{\scriptscriptstyle \rm dS$)~\cite{Cicoli:2012fh}
\begin{align}
V_{\rm tot} = \frac{1}{2s} \left(\sum_i q_{{\rm h},i} |\phi_{{\rm h},i}|^2 + \frac{q_{\scriptscriptstyle \rm dS}{4\pi} \frac{\tau_{\scriptscriptstyle \rm dS}{\mathcal{V}}\right)^2
+ \frac{\left(\kappa_{\scriptscriptstyle \rm dS a_{\scriptscriptstyle \rm dS A_{\scriptscriptstyle \rm dS\right)^2}{s}\frac{e^{-2 a_{\scriptscriptstyle \rm dS \left(s+\kappa_{\scriptscriptstyle \rm dS\tau_{\scriptscriptstyle \rm dS\right)}}{\mathcal{V}} + V_{\mathcal{O}(\mathcal{V}^{-3})}\,,
\label{Vtot1}
\end{align}
where the second term comes from the new superpotential (\ref{Wds2}) and $V_{\mathcal{O}(\mathcal{V}^{-3})}$ is given in~(\ref{eq:LVSScalarPotential}). Minimization with respect to $\tau_{\scriptscriptstyle \rm dS$ gives
\begin{align}
\frac{q_{\scriptscriptstyle \rm dS}{4\pi} \frac{\tau_{\scriptscriptstyle \rm dS}{\mathcal{V}}=- \sum_i q_{{\rm h},i} |\phi_{{\rm h},i}|^2
+ \frac{a_{\scriptscriptstyle \rm dS \kappa_{\scriptscriptstyle \rm dS}{q_{\scriptscriptstyle \rm dS}\left(\kappa_{\scriptscriptstyle \rm dS a_{\scriptscriptstyle \rm dS A_{\scriptscriptstyle \rm dS\right)^2 e^{-2 a_{\scriptscriptstyle \rm dS s}\,.
\label{tdsVEV}
\end{align}
Assuming that model-dependent contributions from F-terms of hidden matter fields fix some $\phi_{{\rm h}, i}$ at non-zero VEVs such that $\langle\sum_i q_{{\rm hid},i} |\phi_{{\rm hid},i}|^2\rangle = 0$ but $A_{\scriptscriptstyle \rm dS\neq 0$,\footnote{In order to make $W_{\scriptscriptstyle \rm dS$ gauge invariant, $A_{\scriptscriptstyle \rm dS$ has to depend on the $\phi_{{\rm h},i}$ which can develop non-zero VEVs for appropriate hidden field F-terms contributions, giving $A_{\scriptscriptstyle \rm dS\neq 0$ with $\tau_{\scriptscriptstyle \rm dS$ in the singular regime~\cite{Cicoli:2012fh}.} and substituting the VEV (\ref{tdsVEV}) in (\ref{Vtot1}) we obtain at leading order
\begin{align}
V_{\rm tot} = V_{D,0}+ \frac{(\kappa_{\scriptscriptstyle \rm dS a_{\scriptscriptstyle \rm dS A_{\scriptscriptstyle \rm dS)^2}{s} \frac{e^{-2 a_{\scriptscriptstyle \rm dS s}}{\mathcal{V}} + V_{\mathcal{O}(\mathcal{V}^{-3})}\,.
\end{align}
Given that the dilaton is fixed by a ratio of flux quanta, the extra positive definite contribution can easily be tuned to obtain a dS minimum. Following~\cite{Cicoli:2012fh}, a cancellation of the vacuum energy at $\mathcal{O}(\mathcal{V}^{-3})$ requires to tune three-form fluxes such that
\begin{align}
\left(\frac{\kappa_{\scriptscriptstyle \rm dS a_{\scriptscriptstyle \rm dS A_{\scriptscriptstyle \rm dS}{W_0}\right)^2 \,e^{-2 a_{\scriptscriptstyle \rm dS s}
...
\begin{align}
V_{D,0} =\frac{1}{2s} \left(\frac{a_{\scriptscriptstyle \rm dS \kappa_{\scriptscriptstyle \rm dS}{q_{\scriptscriptstyle \rm dS}\right)^2\left(\kappa_{\scriptscriptstyle \rm dS a_{\scriptscriptstyle \rm dS A_{\scriptscriptstyle \rm dS\right)^4 e^{-4 a_{\scriptscriptstyle \rm dS s}\sim \mathcal{O}\left(\mathcal{V}^{-4}\right)\,.
\label{VD0ds2}
\end{align}
...
\left(1+3 y_{\scriptscriptstyle \rm dS \epsilon_s+\mathcal{O}(\epsilon_s^2)\right)+\mathcal{O}\left(\frac{1}{\mathcal{V}^2}\right)\right]\,,
\label{expandm32}
\end{align}
where $y_{\scriptscriptstyle \rm dS =1$ for the dS case 1 of Sec.~\ref{sssec:MatterUplift} while $y_{\scriptscriptstyle \rm dS = 1+\frac{\sqrt{2} a_s^{3/4}}{\kappa_{\scriptscriptstyle \rm dS a_{\scriptscriptstyle \rm dS}$ for the dS case 2 of Sec.~\ref{sssec:DilatonUplift}. The leading order F-terms for $T_b$ and $T_s$ turn out to be
\begin{align}
\frac{F^{T_b}}{\tau_b} \simeq - 2 m_{3/2} \left(1+\frac{x_{\scriptscriptstyle \rm dS}{a_s^{3/2} \mathcal{V}\sqrt{\epsilon_s}}\right)\,,
\qquad
\frac{F^{T_s}}{\tau_s} \simeq - 6   m_{3/2} \epsilon_s\,,
\label{FTb}
\end{align}
where $x_{\scriptscriptstyle \rm dS = - 45/16$ for the dS case 1 of Sec.~\ref{sssec:MatterUplift} while $x_{\scriptscriptstyle \rm dS \sim \mathcal{O}(1/\mathcal{V})$ for the dS case 2 of Sec.~\ref{sssec:DilatonUplift}, as reported below. Because of the shift from their supersymmetric minimum, also $S$ and $U$ develop non-vanishing F-terms whose leading order
expressions are
\begin{align}
\label{ds}
...
\end{align}
where $\omega'_S(U,S) \equiv 3 - 2 \omega_S(U,S)$ with $\omega_S$ as defined in (\ref{dilmin}) and $\beta^{U^a}$ are unknown $\mathcal{O}(1)$ functions of $U$ and $S$. Additional non-zero F-terms are associated to fields responsible for achieving a dS solution. For the dS case 1 of Sec.~\ref{sssec:MatterUplift} there is an F-term associated to $\phi_{\scriptscriptstyle \rm dS$
\begin{align}
\frac{F^{\phi_{\scriptscriptstyle \rm dS}}{\phi_{\scriptscriptstyle \rm dS} \simeq m_{3/2}\,,
\end{align}
with $\phi_{\scriptscriptstyle \rm dS$ given in (\ref{CCconddS1}) (up to an irrelevant phase). On the other hand, in the dS case 2 of Sec.~\ref{sssec:DilatonUplift} the blow-up mode $T_{\scriptscriptstyle \rm dS$ has a non-vanishing F-term (using the condition (\ref{CCconddS2}))
\begin{align}
F^{T_{\scriptscriptstyle \rm dS} \simeq \frac{3}{4\sqrt{2}a_s^{3/4}}\frac{m_{3/2}}{\epsilon_s^{1/4}} \,.
\end{align}
Finally, the F-terms associated to the MSSM cycles $T_{\scriptscriptstyle \rm SM$ and $G$ vanish:
\begin{align}
F^G = F^{T_{\scriptscriptstyle \rm SM} = 0\, .
\end{align}
This result is crucial for sequestering since the dominant F-terms are then associated with moduli which couple weakly to the visible sector via Planck-suppressed interactions.\footnote{$T_{\scriptscriptstyle \rm SM$ and $G$ can develop non-zero F-terms only in the presence of tachyonic scalar masses~\cite{Cicoli:2013cha}. However, also in this case, their contribution to soft-terms turns out to be negligible.}
...
\begin{align}
V_{\scriptscriptstyle \rm dS=\frac{r}{\mathcal{V}^m}\qquad\text{with}\quad r>0\quad\text{and}\quad m<3\,.
\end{align}
We are interested in minimizing the combined system
\begin{align}
V=V_{\mathcal{O}(\mathcal{V}^{-3})}+V_{\scriptscriptstyle \rm dS\,,
\end{align}
...
=\frac{\hat{\xi}}{2}\left[1+f_{\scriptscriptstyle \rm dS(\epsilon_s)\right]\,,
\label{eq:shiftds}
\end{align}
and so the function $f_{\scriptscriptstyle \rm dS$ is $f_{\scriptscriptstyle \rm dS= 18 \epsilon_s+297 \epsilon_s^2$ in the case of dS vacua from hidden matter fields ($m=8/3$),
while $f_{\scriptscriptstyle \rm dS=3 \epsilon_s+12 \epsilon_s^2$ for the case of non-perturbative effects at singularities ($m=1$).
...
\begin{align}
\frac{F^{T_b}}{\tau_b}&=-2 m_{3/2}\left[1+\frac{9 \hat{\xi} \epsilon_s}{4 \mathcal{V}} \frac{m-1}{m-3+2m\epsilon_s} +\mathcal{O}\left(\frac{1}{\mathcal{V}^2}\right)\right]\,, \\
\frac{F^{T_s}}{\tau_s}&= -2 m_{3/2}\left[\frac{3\epsilon_s}{(1-\epsilon_s)}-\frac{\hat{\xi}}{2\mathcal{V}}
\left(1-\frac{9\epsilon_s}{2}\frac{m-1}{m-3}+\mathcal{O}\left(\epsilon_s^2\right)\right)\right]\,, \\
\frac{F^S}{s}&= \frac{3 \omega'_S }{8 a_s^{3/2}}  \frac{m_{3/2}}{\mathcal{V}\epsilon_s^{3/2}}\left[1+\mathcal{O}\left(\epsilon_s\right)\right]\,, \\
F^U&= - \frac{K^{U^a \overline{U}_{\overline{b}}}}{2 s^2}\frac{\omega_{\overline{U}_{\overline{b}}}}{\omega'_S}F^S\equiv \beta^{U^a}\,F^S\,, \\
F^{\phi_{\scriptscriptstyle \rm dS} &= \phi_{\scriptscriptstyle \rm dS m_{3/2}\left[1+ \mathcal{O}\left(\frac{1}{\mathcal{V}}\right)\right] \,, \\
F^{T_{{\scriptscriptstyle \rm dS}}&= \frac{3}{4\sqrt{2}a_s^{3/4}}\frac{m_{3/2}}{\epsilon_s^{1/4}}\left[1+\mathcal{O}\left(\epsilon_s\right)\right]\,.
\end{align}

\subsection{Local and Ultra-local Scenarios}
\label{soft}

Our analysis of soft-terms will distinguish between two classes of models: local and ultra-local. This classification is motivated by locality arguments already discussed in Sec.~\ref{sssec:D3atSingularities}: the \Kahler matter metric should scale as $\tilde{K}_\alpha \sim  \mathcal{V}^{-2/3}$ at leading order. In the present section we expand the constraint on the \Kahler matter metric coming from the structure of physical Yukawa couplings in eq. \eqref{eq:HolomorphicYukawaCoupling} at sub-leading order, and we get (for $\tau_{\scriptscriptstyle \rm SM = b= C^\alpha= 0$)
\begin{align}
\label{ul}
\tilde{K}_\alpha = h_\alpha(U,S)\,e^{K/3} \simeq  \frac{h_\alpha(U,S)\,e^{K_{\rm cs}/3}}{(2s)^{1/3}\mathcal{V}^{2/3}} \left(1- \frac{\hat\xi}{3\mathcal{V}}  + \frac 13 K_{\scriptscriptstyle \rm dS\right)\,,
\end{align}
where $h_\alpha(U,S)$ is an unknown function of $U$ and $S$ and in the approximation we focus on the first sub-leading order corrections, e.g.~neglecting higher order corrections of $\mathcal{O}\left(1/\mathcal{V}^{8/3}\right)$. Note that this result has the same volume scaling of our formula for the matter metric (\ref{mattermetric}) which for $\tau_{\scriptscriptstyle \rm SM = b= 0$ reduces to
\begin{align}
\tilde{K}_{\alpha} = \frac{f_\alpha(U,S)}{\mathcal{V}^{2/3}} \left(1 - c_s \frac{\hat\xi}{\mathcal{V}}
+ \tilde{K}_{\scriptscriptstyle \rm dS\right)\equiv f_\alpha(U,S)\tilde{K}\,.
\label{newKtilde}
\end{align}
...
\end{itemize}
If we parametrize $\tilde{K}_{\scriptscriptstyle \rm dS$ as $\tilde{K}_{\scriptscriptstyle \rm dS  =  c_{\scriptscriptstyle \rm dS K_{\scriptscriptstyle \rm dS$,
comparing (\ref{ul}) with (\ref{newKtilde}), we find that in the ultra-local case
\begin{align}
f_{\alpha}(U,S) = \frac{h_\alpha(U,S)\,e^{K_{\rm cs}/3}}{(2s)^{1/3}}\qquad \text{and}\qquad c_s=c_{\scriptscriptstyle \rm dS=\frac 13\,.
\label{fdef}
\end{align}
Subleading deviations from the approximation in~(\ref{ul}) can be accounted for by small changes in $c_s$ and $c_{\scriptscriptstyle \rm dS$ at the appropriate sub-leading order.
...
\end{align}
where $f_a=\delta_a S+ \kappa_a \,T_{\scriptscriptstyle \rm SM$ is the gauge kinetic function as in~\eqref{eq:gaugekineticf}. As $F^{T_{\scriptscriptstyle \rm SM}=0$, we obtain universal gaugino masses, $M_1=M_2=M_3=M_{1/2}$, which are generated by the dilaton F-term. Potential non-universalities can arise through anomaly mediated contributions which turn out to be sub-leading (see Sec.~\ref{sec:DesequesteringSources} for more details). The leading order expression for the gaugino masses is
\begin{align}
M_{1/2} = \frac{F^S}{2 s} \simeq \frac{3 \omega'_S(U,S)}{16 a_s^{3/2}}  \frac{m_{3/2}}{\mathcal{V}\epsilon_s^{3/2}}
\sim \mathcal{O}\left(m_{3/2}\frac{\left(\ln\mathcal{V}\right)^{3/2}}{\mathcal{V}}\right)\ll m_{3/2}\,.
\label{ggmm}
\end{align}
...
\simeq \nonumber \\
&\simeq \frac{15}{2}\left(c_s-\frac{1}{3}\right)\, \frac{m_{3/2}^2 \tau_s^{3/2}}{\mathcal{V}}\sim m_{3/2} M_{1/2}\,,
\label{localscalar}
\end{align}
...
from the F-terms of the dilaton and the complex structure moduli at $\mathcal{O}(\mathcal{V}^{-2})$. Using (\ref{ds}), the $S$ and $U$-dependent contribution to scalar masses turns out to be
\begin{align}
\left.m_\alpha^2\right|_F =  - M_{1/2}^2 s^2 \left(\partial_s^2+\beta^{U^a}\partial_{u^a, s}
+ \beta^{U^a}\beta^{\overline{U}^b}\partial_{u^a,u^b}\right) \ln h_\alpha\sim \mathcal{O}\left(M_{1/2}^2\right)\,,
\label{ulsm3}
\end{align}
...
\label{genscalD}
\end{align}
Given that this result depends on the value of the D-terms potential at the minimum, this contribution depends on the way to achieve a dS vacuum. As explained in Sec.~\ref{sssec:Shrinking}, the VEV of the D-terms potential associated to visible sector $U(1)$s is vanishing in the absence of tachyonic scalars.\footnote{Even in the presence of tachyonic scalars, the contribution to scalar masses from visible sector D-terms turns out to be a negligible effect since visible matter fields, $\tau_{\scriptscriptstyle \rm SM$ and $b$ would still be stabilized at zero at leading order.}
...
\begin{align}
\left.m_0^2\right|_D &= \frac{q_b}{2 f_\alpha(U,S)} D_{\dS_1} \partial_{\tau_b} \tilde{K}_{\alpha}= \frac{m_{3/2}^2}{3s}|\phi_{\scriptscriptstyle \rm dS|^2 = \nonumber \\
&= \frac{6\epsilon_s}{\omega'_S}m_{3/2}M_{1/2}\sim  \mathcal{O}\left(m_{3/2}^2\frac{\sqrt{\ln\mathcal{V}}}{\mathcal{V}}\right)\,,
\label{sc1}
\end{align}
once we impose the condition (\ref{CCconddS1}) to have a vanishing cosmological constant at $\mathcal{O}(\mathcal{V}^{-3})$. In the local limit, this result is suppressed with respect to the F-terms contribution (\ref{localscalar}) by a factor of $\epsilon_s$. On the other hand, in the ultra-local limit, this D-terms contribution dominates over the F-terms one given in (\ref{ulsm3}) which scales as $m_{3/2}^2\epsilon^2$.
...
\begin{align}
\left.m_0^2\right|_D &= \frac{q_{\scriptscriptstyle \rm dS \mathcal{V}^{2/3}}{2 s f_\alpha(U,S)} D_{\dS_2} \partial_{\tau_{\scriptscriptstyle \rm dS} \tilde{K}_{\alpha}-V_{D,0} = \nonumber \\
&= \frac{c_{\scriptscriptstyle \rm dS}{s} D_{\dS_2} q_{\scriptscriptstyle \rm dS\frac{\tau_{\scriptscriptstyle \rm dS}{\mathcal{V}} -V_{D,0} = \left(2c_{\scriptscriptstyle \rm dS-1\right)V_{D,0}\,.
\label{Vds22}
\end{align}
...
Adding $-V_{F,0}/3= V_{D,0}/3$ to (\ref{Vds22}) we find that the total D-terms contribution to scalar masses vanishes in the ultra-local limit once we impose $c_{\scriptscriptstyle \rm dS=1/3$ as in (\ref{fdef}) since
\begin{align}
\left.m_0^2\right|_D =  2\left(c_{\scriptscriptstyle \rm dS-\frac 13\right)V_{D,0}=0 \quad\text{for}\quad c_{\scriptscriptstyle \rm dS=\frac 13\,.
\label{Vds2}
\end{align}
...
&\left.- s \partial_{s,u} \ln \left(\frac{Y_{\alpha \beta \gamma}}{f_\alpha f_\beta f_\gamma} \right) \right] M_{1/2}\sim\mathcal{O}\left(M_{1/2}\right)\,,
\end{align}
with $\partial_{s,u}\equiv \partial_s+\beta^{U^a}\partial_{u^a}$.
Note that there is a cancellation at $\mathcal{O}(\mathcal{V}^{-1})$ between $K$ and $3\ln\tilde{K}$ in (\ref{tril}).
...
\ln \left(\frac{Y_{\alpha \beta \gamma}(U,S)}{h_\alpha h_\beta h_\gamma}\right) M_{1/2}\sim\mathcal{O}\left(M_{1/2}\right)\,.
\end{align}
...
- s \partial_{s,u} \ln\gamma \right] M_{1/2}\sim\mathcal{O}\left(M_{1/2}\right)\,,
\label{mu11}
\end{align}
where again there is a cancellation at $\mathcal{O}(\mathcal{V}^{-1})$ between the term proportional to $m_{3/2}$ in (\ref{muK}) and the leading order contribution from $\overline{F}^{\overline{T}_b}\partial_{\overline{T}_b}Z$. The dominant contributions to (\ref{mu11}) come from the F-terms of $T_b$, $S$ and $U$. On the other hand, the $B\hat\mu$-term behaves as the soft scalar masses since both F- and D-terms contributions can be rewritten as
\begin{align}
...
\begin{align}
B\hat{\mu} = \frac{\gamma}{\sqrt{f_{H_u} f_{H_d}}}\, \frac{5\left(c_s-\frac 13\right)}{\omega'_S} m_{3/2}M_{1/2}\sim \mathcal{O}\left(m_{3/2}^2\frac{\left(\ln\mathcal{V}\right)^{3/2}}{\mathcal{V}}\right)\,.
\label{bmu11local}
\end{align}
...
\begin{align}
\label{fdef2}
\gamma(U,S) = \frac{z(U,S)\,e^{K_{\rm cs}/3}}{(2s)^{1/3}}\qquad \text{and}\qquad c_s=c_{\scriptscriptstyle \rm dS=\frac 13\,.
\end{align}
In this limit the F-term of $T_b$ does not contribute to $\hat\mu$ whose expression simplifies to
\begin{align}
\label{mu2}
\hat\mu  =  - \frac{z\,s\partial_{s,u} \ln\gamma}{\sqrt{h_{H_u } h_{H_d}}}\,M_{1/2}\sim\mathcal{O}\left(M_{1/2}\right)\, .
\end{align}
...
\label{Bmu2}
\end{align}
where $\sigma(U,S)$ is a complicated $\mathcal{O}(1)$ function of $S$ and $U$ which looks like
\begin{align}
...
&= \frac{z}{\sqrt{h_{H_u} h_{H_d}}}\,\frac{6\epsilon_s}{\omega'_S}m_{3/2}M_{1/2}\sim  \mathcal{O}\left(m_{3/2}^2\frac{\sqrt{\ln\mathcal{V}}}{\mathcal{V}}\right)\,,
\label{bmu2dS1}
\end{align}
...
\begin{align}
\left.B\hat\mu\right|_D &= \frac{z}{\sqrt{h_{H_u} h_{H_d}}}\,\left.m_0^2\right|_D = \nonumber \\
&= \frac{z}{\sqrt{h_{H_u} h_{H_d}}}\,2\left(c_{\scriptscriptstyle \rm dS-\frac 13\right)V_{D,0}=0 \quad\text{for}\quad c_{\scriptscriptstyle \rm dS=\frac 13\,,
\label{bmu2dS2}
\end{align}
...
\begin{align}
B\hat\mu = \frac{z}{\sqrt{h_{H_u} h_{H_d}}}\,\sigma(U,S) \,M_{1/2}^2\sim\mathcal{O}\left(M_{1/2}^2\right)\,.
\label{Bmu22}
\end{align}
\end{enumerate}
...
\label{KWphi}
\end{align}
Thus the field $\Phi$ has to be an MSSM singlet since a higgs bilinear gets generated only when $\Phi$ develops a non-zero VEV breaking the $U(1)$ symmetry. However, as can be seen from eqs. (\ref{dtermpot}) and (\ref{xiSM}), D-terms stabilization fixes the VEV of $\Phi$ in terms of $\tau_{\scriptscriptstyle \rm SM$: $|\Phi|^2 \propto \tau_{\scriptscriptstyle \rm SM/\mathcal{V}$, and so the couplings in (\ref{KWphi}) would give rise to effective $\mu$ and $Z$-terms which depend only on closed string moduli
\begin{align}
Z_{\rm eff}\propto \frac{1}{\Lambda^m}\left(\frac{\tau_{\scriptscriptstyle \rm SM}{\mathcal{V}}\right)^{m/2}\,,
\quad \quad\mu_{\rm eff}\propto \frac{1}{\Lambda^{m-1}}\left(\frac{\tau_{\scriptscriptstyle \rm SM}{\mathcal{V}}\right)^{m/2}\,.
\label{Eff}
\end{align}
Once the cut-off $\Lambda$ is explicitly written in terms of $T$-moduli, one could plug (\ref{Eff}) into the standard supergravity formulas to work out the final contribution to $\hat\mu$ and $B\hat\mu$. The result will depend on the VEV and the F-term of $T_{\scriptscriptstyle \rm SM$. As discussed in~\cite{Cicoli:2013cha}, $\Phi$ needs to receive tachyonic contribution from soft-terms in order for $T_{\scriptscriptstyle \rm SM$ to develop a non-zero VEV. If this condition is satisfied, $\tau_{\scriptscriptstyle \rm SM \sim \mathcal{V}^{-1}$ implying $F^{T_{\scriptscriptstyle \rm SM}\sim \mathcal{V}^{-2}$ for the local case
and $\tau_{\scriptscriptstyle \rm SM \sim \mathcal{V}^{-3}$ implying $F^{T_{\scriptscriptstyle \rm SM}\sim \mathcal{V}^{-4}$ for the ultra-local case.
This effect corresponds to switching on an FI-term, and so to breaking the anomalous $U(1)$ by moving slightly away from the singularity. However in both cases the VEV of $\tau_{\scriptscriptstyle \rm SM$ is smaller than unity, and so we are still consistently in the singular regime.\\
...
c_{\mu,W} \frac{M_{\rm P}}{\mathcal{V}^{n + \frac 13}}\, \qquad\,\,\,\,\,\,\,\, \text{(contribution from $W$)}
\end{array}$}
\\ \hline
$B\hat\mu$ & \multicolumn{3}{c|}{$\begin{array}{c}c_{B,K}\,Z\,m_{0}^2\, \qquad  \text{ (contribution from $K$)} \\
c_{B,W} \frac{M^2_{\rm P}}{\mathcal{V}^{n + \frac 43}} \qquad\,\,\,\, \text{(contribution from $W$)}
\end{array}$}
\\
\hline
\end{tabu}}
...
The size of loop corrections can be estimated by realizing that, if SUSY is broken, loop corrections to the mass should be given by the heaviest particles circulating in the loop (or the cut-off scale) which is the Kaluza-Klein scale $M_{\scriptscriptstyle \rm KK\sim M_{\rm P}/\mathcal{V}^{2/3}$. In the absence of SUSY there is a need of a SUSY-breaking insertion (a spurion field representing the relevant F-term) in the loop and the correction to the mass is at most
\begin{align}
\delta m=\alpha_{\scriptscriptstyle \rm loop \frac{M_{\scriptscriptstyle \rm KK m_{3/2}}{M_{\rm P}}\sim \alpha_{\scriptscriptstyle \rm loop\frac{W_0}{\mathcal{V}^{5/3}}\ll \alpha_{\scriptscriptstyle \rm loop m_{3/2}\,,
\end{align}
with $\alpha_{\scriptscriptstyle \rm loop\sim g^2/(16\pi^2)$ a loop factor. Note that the ratio $\delta m/m\sim \alpha_{\scriptscriptstyle \rm loop \mathcal{V}^{-1/6}$ is very small and therefore the volume modulus mass is stable against loop corrections.\\
...
and general gravity mediated models. See for example~\cite{Ellis:1986nr,Antoniadis:1997ic} in which loop corrections to scalar and gaugino masses were estimated in supergravity and M-theory frameworks with results of order $\delta m \sim \alpha_{\scriptscriptstyle \rm loop m_{3/2}^2/M_{\rm P}\sim \alpha_{\scriptscriptstyle \rm loop M_{\rm P}/\mathcal{V}^2$. More recently, explicit calculations for gravitino loop contributions to gaugino masses was performed in~\cite{Lee:2013aia}. The diagrams are quadratically divergent and proportional to the gravitino mass:
\begin{align}
\delta M_{1/2} = \frac{m_{3/2}}{16\pi^2}\left(\frac{\Lambda^2}{M_{\rm P}^2}+\dots\right)\,,
\end{align}
where $\Lambda$ is the cut-off scale and the dots represent sub-leading logarithmically divergent terms. In string theory we expect that $\Lambda\leq M_s\sim M_{\rm P}/\mathcal{V}^{1/2}$ which then corrects the gaugino masses to order $\delta M_{1/2}\leq \alpha_{\scriptscriptstyle \rm loop M_{\rm P}/\mathcal{V}^2$ which is smaller than the sequestered gaugino masses $M_{1/2}\sim M_{\rm P}/\mathcal{V}^2$.\\
...
$\omega_S' \textrightarrow 0$, i.e. in the very tuned situation where the F-term of the dilaton is vanishing at leading order because of a special compensation between the contribution to $F^S$ from $D_S W$ and $D_{T_b} W$. In this case the leading contribution to gaugino masses given in (\ref{ggmm}) is zero and the first non-vanishing moduli mediated contribution can be estimated to scale as $M_{1/2}^{\rm new}\sim m_{3/2} \sqrt{\ln\mathcal{V}}/\mathcal{V} $. On the other hand, the anomaly mediated contribution scales as $M_{1/2}^{\rm anom}\sim c \,M_{1/2}^{\rm new}$ where $c = c' \left(\frac{g^2}{16\pi^2}\right) \ln\mathcal{V}$ and $c'$ denotes a numerical factor arising from evaluating~\eqref{anorel2} exactly. For $g\simeq 0.1$ and $\mathcal{V} \simeq 5\cdot 10^6$ (the value needed to get $M_{1/2}^{\rm new}$ approximately around the TeV-scale), $c$ is roughly of order $c'\times10^{-3}.$ Depending on the exact value of $c',$ which is beyond the scope of this analysis, we can achieve competing contributions from moduli mediation and anomaly mediation.

\subsection{Moduli Redefinitions}

Desequestering can also potentially occur due to moduli redefinitions which might be necessary order by order in perturbation theory. This desequestering effect can for example arise due to a shift of the local cycle $\tau_{\scriptscriptstyle \rm SM\rightarrow \tau_{\scriptscriptstyle \rm SM+\alpha\ln\mathcal{V}$ which has the effect of making the soft-terms of the same order as the gravitino mass~\cite{Conlon:2010ji,Choi:2010gm}.\\
...
Generically in string compactifications SUSY-breaking effects develop a mass for the moduli and generate by gravity or anomaly mediation soft-terms of order $M_{\rm soft}$. Due to their common origin, the mass of the lightest modulus $m_{\rm mod}$ is therefore related to the scale of the soft-terms as $M_{\rm soft} = \kappa m_{\rm mod}$. Given the cosmological constraint $m_{\rm mod}\gtrsim 1.3 \times 10^5$ GeV, only models with $\kappa\ll 1$ can allow for low-energy SUSY to solve the hierarchy problem. Values of $\kappa\sim\mathcal{O}(10^{-2})$ can come from loop suppression factors~\cite{Choi:2005ge, Lowen:2008fm, Acharya:2008zi,Dudas:2012wi} while much smaller values $\kappa\sim\mathcal{O}(10^{-3}-10^{-4})$ can arise due to sequestering effects~\cite{Aparicio:2014wxa, Blumenhagen:2009gk}. For $M_{\rm soft}\sim \mathcal{O}(1)$ TeV, the corresponding reheating temperature becomes
\begin{align}
T_R \sim  \frac{M_{\rm soft}}{\kappa^{3/2}} \sqrt{\frac{M_{\rm soft}}{M_{\rm P}}} \sim \,\kappa^{-3/2}\,\mathcal{O}(10^{-2})\,\,{\rm MeV}\,,
\end{align}
which for $10^{-2}\lesssim\kappa\lesssim 10^{-4}$ is between $\mathcal{O}(10)$ MeV and $\mathcal{O}(10)$ GeV. This is below the freeze-out temperature for LSP masses between $\mathcal{O}(100)$ GeV and $\mathcal{O}(1)$ TeV which is
...
\begin{align}
\mathcal{L}_{\rm kin} = \frac 12 \partial_\mu \hat\Phi \partial^\mu \hat\Phi + \frac 12 \partial_\mu a \partial^\mu a - \sqrt{\frac 23 } \frac{\hat \Phi}{M_{\rm P}} \partial_\mu a \partial^\mu a\,,
\end{align}
...
The dominant volume modulus decays into visible sector \textit{dof} are to higgses via the GM coupling $Z$. Additional leading order decay channels can be to SUSY scalars and higgsinos depending respectively on $m_0$ and $\hat\mu$. On the other hand, as explained in~\cite{Cicoli:2012aq,Higaki:2012ar}, $\tau_b$ decays into visible gauge bosons are loop suppressed, i.e. $c_{\Phi\textrightarrow A A}\sim \alpha_{\scriptscriptstyle \rm SM^2 \ll 1$, whereas decays into matter fermions and gauginos are chirality suppressed, i.e. $c_{\Phi\textrightarrow f f}\sim \left(m_f/m_\Phi\right)^2 \ll 1$. The main goal of this section is to compute the cubic interaction lagrangian which gives rise to the decay of the volume modulus into higgses, higgsinos, squarks and sleptons.
...
\begin{align}
\mathcal{L}_{\Phi h h} =  - \frac{1}{M_{\rm P}\sqrt{6}} \left[\partial_\mu \hat\Phi h_i\partial^\mu h^i + \hat\Phi \partial_\mu h_i \partial^\mu h^i
+Z\partial_\mu \hat\Phi \sum_{i=1}^4 (-1)^{i+1}\left(h_{2i}\partial^\mu h_{2i-1} + h_{2i-1}\partial^\mu h_{2i} \right)\right], \nonumber
\end{align}
and:
\begin{align}
\mathcal{L}_{\Phi C C} = - \frac{1}{M_{\rm P}\sqrt{6}} \left(\sigma_\alpha \partial_\mu \sigma^\alpha \partial^\mu \hat\Phi + \chi_\alpha \partial_\mu \chi^\alpha \partial^\mu \hat\Phi + \hat\Phi \partial_\mu \sigma_\alpha \partial^\mu \sigma^\alpha + \hat\Phi \partial_\mu \chi_\alpha \partial^\mu \chi^\alpha\right)\,. \nonumber
\end{align}
...
\begin{align}
\mathcal{L}_{\rm cubic} = \frac{1}{M_{\rm P}\sqrt{6}} \left[ \hat \Phi h_i \Box h^i + \hat \Phi \left(\sigma_\alpha \Box \sigma^\alpha + \chi_\alpha \Box \chi^\alpha \right) + \left(\alpha \hat{\mu}^2 + \beta m_0^2 \right) \, \hat \Phi h_i h^i\right. + \nonumber \\
...
\begin{align}
\mathcal{L}_{\rm cubic} = - \frac{1}{M_{\rm P}\sqrt{6}} &\left[\left(\hat{\mu}^2 \left(1 - \alpha\right) + m_0^2 \left(1 - \beta \right)\right) \, \hat\Phi h_i h^i 
...
\begin{align}
\mathcal{L}_1 = \lambda \frac{m^2}{M_{\rm P}}\Phi \phi \phi \quad\Rightarrow \quad
...
\mathcal{L}_2 = \lambda \frac{m^2}{M_{\rm P}}\Phi \phi_1 \phi_2\,\,\Rightarrow \,\,
...
\begin{align}
\mathcal{L} \supset i \tilde{H}^{\dagger}_i \overline{\sigma}^\mu \partial_\mu \tilde{H}^{i} \left(1-\sqrt{\frac 23}\frac{\hat\Phi}{M_{\rm P}}\right)- \frac{\hat\mu}{2} \left(\tilde{H}_u^+ \tilde{H}_d^- - \tilde{H}_u^0 \tilde{H}_d^0\right)\left(1-\frac{\alpha}{\sqrt{6}} \frac{\hat\Phi}{M_{\rm P}}\right) +\text{h.c.} \,.
\end{align}
After imposing the equations of motion, we get the following cubic interaction lagrangian:
\begin{align}
\mathcal{L}_{\rm cubic} \supset \, \frac{\alpha}{2 \sqrt{6}} \, \frac{\hat\mu}{M_{\rm P}} \,\hat\Phi \left(\tilde{H}_u^+ \tilde{H}_d^- - \tilde{H}_u^0 \tilde{H}_d^0\right)+\text{h.c.}\,.
\label{higgsinocubic}
\end{align}
...
\begin{align}
m_0^2 \simeq M_{1/2}^2 \simeq B\hat{\mu} \simeq \hat{\mu}^2 \sim \frac{M_{\rm P}^2}{\mathcal{V}^4} \ll m_{\Phi}^2 \sim \frac{M_{\rm P}^2}{\mathcal{V}^3}\,.
\end{align}
...
\begin{align}
\label{hierarchyss}
M_{1/2}^2 \simeq \hat{\mu}^2 \sim \frac{M_{\rm P}^2}{\mathcal{V}^4} \ll m_0^2 \simeq B\hat\mu \simeq m_{\Phi}^2 \sim \frac{M_{\rm P}^2}{\mathcal{V}^3}\,.
\end{align}
...
\begin{align}
\mathcal{L}_{\rm cubic} \supset &\frac{Z}{2\sqrt{6}}\, \frac{m_\Phi^2}{M_{\rm P}} \, 
...
\item \emph{Local dS$_2$ case}: The situation seems more promising in this case since in the local limit the D-terms contribution to $m_0^2$ is volume-suppressed with respect to $m_\Phi^2$ since it scales as $\left. m_0^2\right|_D \sim \mathcal{O}\left(\mathcal{V}^{-4}\right)$~\cite{Aparicio:2014wxa}. In this case it is therefore possible to tune the coefficient $c_s$ to obtain $R\leq 1/4$. By comparing the second term in~\eqref{vmm} with~\eqref{localscalar}, this implies that $c_s$ has to be tuned so that $\left(c_s - \frac 13\right) \leq \frac{9}{10 \, a_s \tau_s}$, where $a_s \tau_s \sim 80$ in order to get TeV-scale gaugino masses~\cite{Aparicio:2014wxa}. However the condition $m_0^2 > 0$ to avoid tachyonic masses translates into $\left(c_s - \frac 13\right)>0$, giving rise to a very small window: 
\begin{align}
...
\begin{align}
\mathcal{L}_{\rm cubic} \simeq &\frac{7 c^2}{2\sqrt{6}}\frac{m_\Phi^2}{M_{\rm P}} \hat\Phi\left[  
...
\item The KKLT bound $H\lesssim m_{3/2}$ and the LVS bound $H\lesssim m_{3/2}\sqrt{\frac{m_{3/2}}{M_{\rm P}}}$ apply just to the inflationary era, and so in these expression $m_{3/2}$ is the gravitino mass during inflation which might be different from the present value of the gravitino mass. This is possible if $m_{3/2}= e^{K/2} |W| \simeq \frac{W_0}{\mathcal{V}} M_{\rm P}$ evolves just after the end of inflation. Two viable realizations include inflationary models where the inflaton coincides with the volume mode $\mathcal{V}$ so that $\mathcal{V}$ relaxes from small to large values during inflation~\cite{Conlon:2008cj} or where the inflaton is a complex structure modulus or a matter field so that $W$ relaxes from large to small values during inflation~\cite{He:2010uk}. Another option is to consider models with two different volume stabilization mechanisms during and after inflation. If $\mathcal{V}$ couples to the field $X$ whose F-term drives inflation, the volume's vacuum expectation value (VEV) during inflation would be determined by the F-terms potential of $X$ which however vanishes after the end of inflation when $\mathcal{V}$ is fixed by a more standard KKLT or LVS mechanism. This kind of models with a K\"ahler potential coupling between $\mathcal{V}$ and $X$ have been studied in~\cite{Antusch:2011wu} whereas superpotential interactions between $\mathcal{V}$ and $X$ have been analyzed in~\cite{Yamada:2012tj}.
...
\begin{itemize}
\item In order to have an evolving gravitino mass, we focus on models where the inflaton is the volume mode $\mathcal{V}$. Given that we work in an effective supergravity theory where the K\"ahler potential has a logarithmic dependence on $\mathcal{V}$, each term in the inflationary potential will be a negative exponential of the canonically normalized volume mode $\Phi\sim \ln\mathcal{V}$, i.e. $V \supset e^{- k \Phi}$. This form of the potential is reminiscent of Starobinsky-like models which have a rather large inflationary scale since they are at the boarder between large and small field models~\cite{Starobinsky:1980te}. However Starobinsky-like potentials feature also an inflaton-independent constant which can never appear in cases where the inflaton is the volume mode $\mathcal{V}$ since $\mathcal{V}$ couples to all sources of energy because of the Weyl rescaling needed to obtain the correct effective action.\footnote{Starobinsky-like models with large $r$ and a constant inflaton-independent constant can instead be obtained in models where the inflaton is a K\"ahler modulus different from the volume mode~\cite{Cicoli:2008gp}.}
...
In this setup we have a single K\"ahler modulus $T_b = \tau_b + {\rm i} c_b$ where $c_b$ is an axion field and $\tau_b$ controls the overall volume: $\mathcal{V} = \tau_b^{3/2}$. The canonical normalization of the inflaton field $\tau_b$ can be inferred from its kinetic terms: 
\begin{align}
\mathcal{L}_{\rm kin} = \frac{3}{4 \tau_b^2} \partial^\mu \tau_b \partial_\mu \tau_b\,,
\end{align}
so that the canonically normalized volume field can be written as:
\begin{align}
\Phi = \sqrt{\frac{3}{2}} \ln \tau_b \simeq \sqrt{\frac{2}{3}} \ln \mathcal{V}\,.
\label{CanNorm}
\end{align}
...
\begin{itemize}
\item[i)] The visible sector lives on a stack of D7-branes wrapping $\tau_b$ (for an explicit CY construction see~\cite{Cicoli:2011qg}). As explained above, in this case $\mathcal{V}$ has to be of order $100$ for phenomenological reasons and the SUSY scale is very high.

\item[ii)] The visible sector lives on a stack of D7-branes wrapping $\tau_s$ (see again~\cite{Cicoli:2011qg} for an explicit model). Given that the visible sector gauge coupling is now independent on $\tau_b$ since $\alpha_{\rm vis}^{-1}=\tau_s$, the volume can take large values. A particularly interesting value is $\mathcal{V}\simeq 10^{14}$ which leads to TeV-scale soft-terms, as in standard non-sequestered LVS models. 

\item[iii)] The visible sector is localized on D3-branes at singularities (see~\cite{Cicoli:2012vw, Cicoli:2013cha} for explicit global dS models) obtained by collapsing a blow-up mode to zero size due to D-terms stabilization. Since the gauge kinetic function of D3-branes is given by $S$, $\mathcal{V}$ can take large values. Due to sequestering effects, in this case TeV-scale superpartners require $\mathcal{V}\simeq 10^7$.
\end{itemize}
\end{itemize}
...
\begin{align}
V = \frac{g_s}{8 \pi} \left(\frac{8  a_s^2 A_s^2 \sqrt{\tau_s} e^{-2 a_s \tau_s}}{3 \mathcal{V}} - \frac{4 W_0 a_s A_s \tau_s e^{-a_s \tau_s}}{\mathcal{V}^2} + \frac{3 \hat{\xi} W_0^2}{4 \mathcal{V}^3}\right).
\label{eq:LVSScalarPotentialGs}
\end{align}
Integrating out $\tau_s$ from the scalar potential in eq. \eqref{eq:LVSScalarPotentialGs} we get an effective potential which depends only on the volume $\mathcal{V}$:
\begin{align}
V = \frac{V_0}{\mathcal{V}^3} \left[1 - \frac{2}{\hat\xi a_s^{3/2}} \left(\ln \mathcal{V}\right)^{3/2}\right]\,.
...
Depending on the details of the compactification space, these new terms can take different forms, yielding contributions to the scalar potential that scale differently with the overall volume. In the simplest single K\"ahler modulus case where $t_b = \sqrt{\tau_b} \simeq \mathcal{V}^{1/3}$, the correction \eqref{v1} takes the form:
\begin{align}
V_{F^4} =  \frac{\kappa_{F^4}\,V_0}{\mathcal{V}^{11/3}} = \kappa_{F^4}\, V_0\,e^{-\frac{11}{\sqrt{6}} \Phi}\,,
\label{VF4}
\end{align}
where we defined:
\begin{align}
...
\begin{align}
\mathcal{V} = \frac{1}{6} (3 t_1^2 t_5 + 18 t_1 t_5^2 + 26 t_5^3)\,,
\end{align}
implying that the four-cycles are given by:
\begin{align}
\tau_1 = \frac{\partial \mathcal{V}}{\partial t_1} = t_5 (t_1 + 3 t_5) \quad\text{and}\quad \tau_5 = \frac{\partial \mathcal{V}}{\partial t_5} = \frac{1}{2} (t_1 + 6 t_5)^2\,.
\end{align}
...
\begin{align}
\mathcal{V} = \frac{1}{9 \sqrt{2}} \left(\tau_b^{3/2} - \tau_s^{3/2}\right),
\end{align}
where we identified $\tau_5 \equiv \tau_b$ and $\tau_4 \equiv \tau_s$. Thus the $\alpha'$-correction of \eqref{v1} becomes:
\begin{align}
V_{F^4} \simeq V_0 \left(\frac{\kappa_{F^4_{(b)}}}{\mathcal{V}^{11/3}} + \frac{\kappa_{F^4_{(s)}} \sqrt{\tau_s}}{\mathcal{V}^4}\right),
\label{deltav1}
\end{align}
where:
\begin{align}
...
\begin{align}
V_{g_s} = \sum_{i = 1}^{h^{(1,1)}} \frac{|W_0|^2}{\mathcal{V}^2} \left(g_s^2 \left(\mathcal{C}^{\rm KK}_i\right)^2 \frac{\partial^2 K_0}{\partial \tau_i^2} - 2 \delta K^{\rm W}_{(g_s)}\right).
...
\begin{align}
V_{g_s} = \frac{\kappa_{g_s}\,V_0}{\mathcal{V}^{10/3}} = \kappa_{g_s}\,V_0\,e^{-\frac{10}{\sqrt{6}} \Phi}\,,
\label{Vgs}
\end{align}
...
\begin{align}
V_{\overline{D3}} = \frac{\kappa_{\overline{D3}}\,V_0}{\mathcal{V}^2} = \kappa_{\overline{D3}}\,V_0\,e^{-\sqrt{6}\Phi}\,,
\label{VD3}
\end{align}
...
\begin{align} 
\xi_b = - \frac{q_b}{4 \pi} \frac{\partial K_0}{\partial T_b} = \frac{3 q_b}{8 \pi} \frac{1}{\mathcal{V}^{2/3}}\,,
\end{align} 
...
\begin{align}
V = V_D + c m_{3/2}^2 |\phi|^2 + \mathcal{O}\left(\mathcal{V}^{-3}\right),
\end{align}
where $c$ is an $\mathcal{O}(1)$ coefficient which can be positive or negative depending on hidden sector model building details.
...
\begin{align}
V_{\rm hid} = \frac{\kappa_{\rm hid}\,V_0}{\mathcal{V}^{8/3}} = \kappa_{\rm hid}\,V_0\,e^{-\frac{8}{\sqrt{6}}\Phi}\,,
\label{vds2}
\end{align}
...
\label{eq:unseq}
\end{align}
This is usually referred to as the non-sequestered case which for $W_0\sim 1$ leads to TeV-scale supersymmetry only for values of the volume as large as $\mathcal{V}\sim 10^{14}$. This is possible for the two moduli case where the value of the visible sector coupling does not depend on $\mathcal{V}$. In the single modulus case, since $\alpha_{\rm vis}^{-1}=\mathcal{V}^{2/3}$, the volume has to be of order $100$, resulting necessarily in a high-scale SUSY scenario.
...
\begin{align}
\delta \equiv \sqrt{\frac{g_s}{2}} \frac{W_0}{\mathcal{V}^{1/3} } \ll 1\,.
\end{align}
This bound is satisfied in each case of Tab.~\ref{tab1} and~\ref{tab2} both around the inflection point and the late-time minimum. In fact, considering just the region around the inflection point ($\mathcal{V}$ becomes larger around the minimum and so this bound is stronger during inflation) and setting $g_s=0.1$, we have: $\delta\simeq 10^{-3}$ for $W_0=0.1$, $\delta\simeq 10^{-2}$ for $W_0=1$, $\delta\simeq5\cdot 10^{-2}$ for $W_0=10$ and $\delta\simeq 0.1$ for $W_0=100$.
