...
\def\pref#1{(\ref{#1})}
\newcommand{m_{\scriptscriptstyle A}}{m_{\scriptscriptstyle A}}
\newcommand{\mathbf}{m_{\scriptscriptstyle B}}
...
\end{equation}
where  $\alpha$, $\gamma_1$ and $\gamma_2$ are constants\footnote{For explicit realisations in weighted projective spaces see  e.g. \cite{Blumenhagen:2007sm}.}. In the large volume limit, $\mathcal{V} \simeq \tau_0^{3/2} \gg 1$, we can work perturbatively in $\epsilon \equiv \tau_0^{-1} \ll 1$. The \Kahler metric can be expanded as
\begin{equation}
...
%
\begin{equation}
\mathcal{L} \supset \frac{\sigma \varphi_2}{\langle\mathcal{V}\rangle^{1/2} } F^a_{\mu \nu} F^{a, \mu \nu} \,,
\end{equation}
%
where
%
...
\begin{equation}
\label{supr}
  \Lambda \sim M_{\rm pl} \mathcal{V}^{1/2}.
\end{equation}
%
...
%
\begin{equation}
K_{\alpha \beta} \sim \frac{\tau_1^{1/3}}{\mathcal{V}^{2/3}} \delta_{\alpha \beta} \simeq \frac{\tau_1^{1/3}}{\tau_0} \left(1 + \frac{2}{3} \frac{\tau_1^{3/2}}{\tau_0^{3/2}} + \frac{2}{3} \frac{\tau_2^{3/2}}{\tau_0^{3/2}}\right) \, \delta_{\alpha \beta} \,.
\end{equation}
%
...
\begin{equation}
\label{matmet}
K_{\alpha \beta} = \frac{\langle \tau_1 \rangle^{1/3}}{\langle \mathcal{V} \rangle^{2/3}} \left(1 + \frac{1}{3} \frac{\delta \tau_1}{\langle \tau_1 \rangle} - \frac{\delta \tau_0}{\langle \tau_0 \rangle} + \frac{\gamma_1 \langle \tau_1 \rangle^{1/2}}{\langle \tau_0 \rangle^{3/2}} \delta \tau_1 + \frac{\gamma_2 \langle \tau_2 \rangle^{1/2}}{\langle \tau_0 \rangle^{3/2}} \delta \tau_2\right) \,,
\end{equation}
where $\langle \mathcal{V} \rangle = \alpha\left(\langle\tau_0\rangle^{3/2} - \gamma_1 \langle\tau_1\rangle^{3/2} - \gamma_2 \langle\tau_2\rangle^{3/2}\right)$.  The Lagrangian for matter fields, in particular the electron $e$ is given by
%
\begin{equation}
...
\begin{equation}
e^{K/2} \simeq  \mathcal{V}^{-1} \simeq \frac{1}{\langle \mathcal{V} \rangle} \left(1 - \frac{3}{2} \frac{\delta \tau_0}{\langle \tau_0 \rangle} + \frac{3}{2} \frac{\gamma_1 \langle \tau_1 \rangle^{1/2}}{\langle \tau_0 \rangle^{3/2}} \delta \tau_1 +  \frac{3}{2} \frac{ \gamma_2 \langle \tau_2 \rangle^{1/2}}{\langle \tau_0 \rangle^{3/2}} \delta \tau_2 + \mathcal{O}\left(\mathcal{V}^{-2}\right)\right) \,.
\end{equation}
Hence
\begin{align}
...
&+ \frac{\tilde{y} H}{\langle \mathcal{V} \rangle} \overline{e} e \left(1 - \frac{3}{2} \frac{\delta \tau_0}{\langle \tau_0 \rangle} + \frac{3}{2} \frac{\gamma_1 \langle \tau_1 \rangle^{1/2}}{\langle \tau_0 \rangle^{3/2}} \delta \tau_1 +  \frac{3}{2} \frac{\gamma_2 \langle \tau_2 \rangle^{1/2}}{\langle \tau_0 \rangle^{3/2}} \delta \tau_2\right) \,.
\end{align}
where $K_0 = \langle \tau_1 \rangle^{1/3}/\langle \mathcal{V} \rangle^{2/3}$. After canonical normalization of the electron field: $\hat{e} = \sqrt{K_0} e$ and electroweak symmetry breaking, the Lagrangian takes the form
\begin{align}
...
\end{align}
where $m_e$ is the electron mass. %$ = \frac{\tilde{y} v}{\langle \tau_1 \rangle^{1/3} \langle \mathcal{V} \rangle^{1/3}}$ ($v$ denotes the VEV of H). 
...
\begin{equation}
\label{ecoup}
\mathcal{L}_{\rm int} \supset - \left(\frac{\sigma_0}{\langle \mathcal{V} \rangle^{5/6}} \frac{\varphi_2}{M_{\rm p}} + \frac{\sigma_1}{\langle \mathcal{V} \rangle^{1/2}} \frac{\varphi_2}{M_{\rm p}} + \frac{\sigma_2}{\langle \mathcal{V} \rangle^{1/2}} \frac{\varphi_2}{M_{\rm p}} \right) m_e \overline{\hat{e}} \hat{e} \,,
\end{equation}
where
%
\begin{equation}
...
\begin{equation}
\label{singuni}
K = - 2 \log\left(\alpha \left(\tau_0^{3/2} - \gamma_2 \tau_2^{3/2}\right) + \frac{\xi s^{3/2}}{2}\right) - \log \left( S + \bar{S}\right) + \frac{\tau_1^2}{\mathcal{V}} \,,
\end{equation}
%
...
\begin{equation}
\label{singloop}
K = - 2 \log\left(\alpha \left(\tau_0^{3/2} - \gamma_2 \tau_2^{3/2}\right)\right)   - \log \left( S + \bar{S}\right)+ \frac{\tau_1^2}{\mathcal{V}} + { {\hat{C}  \tau^{1/2}_{2}}\over {\left(S + \bar{S}\right) \tau_{0}^{3/2}} }.
\end{equation}
%
...
%
\begin{equation}
\mathcal{L} \supset - \frac{\zeta \langle s\rangle^{5/2}}{\langle\mathcal{V}\rangle^{3/2}}\, \varphi_2 F^a_{\mu \nu} F^{a,\,\mu \nu} \,,
\end{equation}
%
where
%
...
\begin{equation}
\label{supr}
  \Lambda \sim M_{\rm pl} \frac{\langle\mathcal{V}\rangle^{3/2}}{\langle s \rangle^{5/2}}.
\end{equation}
%
...
%
\begin{equation}
\mathcal{L} \supset \frac{\zeta}{\langle\mathcal{V}\rangle^{1/2}} \, \varphi_2 F^a_{\mu \nu} F^{a,\,\mu \nu} \,,
\end{equation}
%
where
%
...
\begin{equation}
\label{supr}
  \Lambda \sim M_{\rm pl} \langle\mathcal{V}\rangle^{1/2}.
\end{equation}
%
\noindent{{\underline{Coupling to Matter Fields}}}\\
For matter fields arising from D3-branes at singularities, in the limit $\tau_0 \gg 1$, the \Kahler matter metric is given by $K_{\alpha \beta} = \mathcal{V}^{-2/3}$ \cite{Conlon:2006tj, Aparicio:2008wh, Aparicio:2015psl}. On expanding the fields as $\tau_i = \langle \tau_i \rangle + \delta \tau_i$ one finds:
%
\begin{equation}
K_{\alpha \beta} = \frac{\delta_{\alpha \beta}}{\langle \mathcal{V} \rangle^{2/3}} \left(1  - \frac{\delta \tau_0}{\langle \tau_0 \rangle} + \frac{\gamma_2 \langle \tau_2 \rangle^{1/2}}{\langle \tau_0 \rangle^{3/2}} \delta \tau_2\right) \,,
\end{equation}
where $\langle \mathcal{V} \rangle = \alpha \left(\langle \tau_0 \rangle^{3/2} - \gamma_2 \langle \tau_2 \rangle^{3/2} \right)$.  This give the interaction term involving the electrons can be computed as
...
\begin{equation}
\mathcal{L}_{\rm int} \supset \left(\frac{\sigma_0}{\langle \mathcal{V} \rangle^{5/6}} \frac{\varphi_2}{M_{\rm p}} + \frac{\sigma_2}{\langle \mathcal{V} \rangle^{1/2}} \frac{\varphi_2}{M_{\rm p}} \right) m_e \overline{\hat{e}} \hat{e} \,,
\end{equation}
where
%
\begin{equation}
...
%
\begin{equation}
  \mathcal{V} \gtrsim 10^4/g_s^{5/3} \simeq 5 \times 10^5\,, \quad \text{in the case of mixing from the universal $\alpha'$-corrections} \,,
\end{equation}
...
\mathcal{L} \supset \frac{\sigma \varphi_1}{\langle\mathcal{V}\rangle^{1/14} } F^a_{\mu \nu} F^{a, \mu \nu} \,,
\end{equation}
%
where
%
...
\begin{equation}
\label{mthsupr}
  \Lambda \sim M_{\rm pl} \mathcal{V}^{1/14}.
\end{equation}
%
Given the low power of $\mathcal{V}$ in Eq. \pref{mthsupr}, $\varphi_2$ has essentially Planck suppressed couplings with gluons for realistic value of the volume; thus cannot be very light.
...
\begin{equation}
\label{fib}
  K = -2 \log \mathcal{V} = - 2 \log \left(\hat{\mathcal{V}}  -  \gamma \tau_1^{3/2}\right) \,  \phantom{abc} { \text{with}}  \phantom{abc} \hat{\mathcal{V}}  = \alpha \sqrt{\tau_2} \left( \tau_0 - \beta \tau_2 \right) \,,
\end{equation}
%
...
%
\begin{equation}
\mathcal{V} \simeq \alpha \sqrt{\tau_2} \tau_0 \ . 
\end{equation}
%
Using the basis $\{\tau_2, \mathcal{V}, \tau_1\}$ the kinetic matrix can be written as
\begin{equation}
K_{ij} = K^{(0)}_{ij} + K^{(1)}_{ij} \,,
\end{equation}
 where $\delta = \mathcal{V}^{-1}$ and
\begin{equation}
K_{ij}^{(0)} = \begin{pmatrix}
A & 0 & 0 \\
0 & B \delta^2 & 0 \\
...
%
\begin{equation}
\mathcal{L} \supset \frac{\zeta \varphi_2}{\langle\mathcal{V}\rangle} F^a_{\mu \nu} F^{a,\,\mu \nu} \,,
\end{equation}
%
where
%
...
\begin{equation}
\label{supr}
  \Lambda \sim M_{\rm pl} \langle\mathcal{V}\rangle.
\end{equation}
%
\noindent{\underline{Couplings to Matter Fields}}\\ 
...
\begin{equation}
\mathcal{L}_{\rm int} \supset m_e \overline{\hat{e}} \hat{e} \left(-\frac{1}{3} \frac{\delta \mathcal{V}}{\langle \mathcal{V} \rangle} - \frac{1}{3} \frac{\delta \tau_1}{\langle \tau_1 \rangle}\right) \,,
\end{equation}
which, after the basis change leads to 
\begin{equation}
\mathcal{L}_{\rm int} \supset \left(\frac{\sigma_1}{\langle \mathcal{V} \rangle^2} \frac{\varphi_2}{M_{\rm p}} + \frac{\sigma_2}{\langle \mathcal{V} \rangle} \frac{\varphi_2}{M_{\rm p}} \right) m_e \overline{\hat{e}} \hat{e} \,,
\end{equation}
where
%
\begin{equation}
...
\begin{equation}
\label{fib}
  K = - 2 \log \left( {\mathcal{V}} + \frac{\xi s^{3/2}}{2}\right) - \log\left(2s\right)  + \frac{\tau_1^2}{\mathcal{V}} \,,
\end{equation}
with the volume as in Eq. \eqref{fib}. In the basis $\{\tau_2, \mathcal{V}, s\}$, the kinetic matrix can be written as
%
$$
  K_{ij} = K_{ij}^{(0)} + K_{ij}^{(1)} + K_{ij}^{(2)} + ...
$$
...
%
\begin{equation}
K = - 2 \log \hat{\mathcal{V}} - \log\left(2s\right) - \frac{\lambda_2}{s \tau_2} \,.
\end{equation}
%
 Using the basis $\{\tau_2, \mathcal{V}, s\}$, in the regime $\tau_0 \gg \tau_f \gg 1$, the kinetic matrix can be written as
%
\begin{equation}
K_{ij} = K^{(0)}_{ij} + \epsilon_1 K^{(1)}_{ij} + \epsilon_2 K^{(2)}_{ij} \,,
\end{equation}
%
where $\epsilon_1 = \frac{1}{4 \tau_2^2 s^2}$, $\epsilon_2 = \mathcal{V}^{-1}$ and $\epsilon_2 \ll \epsilon_1$. With
\begin{equation}
...
%
\begin{equation}
\mathcal{L} \supset \frac{\zeta \varphi_2}{\langle\mathcal{V}\rangle^3} F^a_{\mu \nu} F^{a,\,\mu \nu} \,,
\end{equation}
%
where
%
...
\begin{equation}
\label{supr}
  \Lambda \sim M_{\rm pl} \langle\mathcal{V}\rangle^3.
\end{equation}
%
...
%
  \begin{equation}
\mathcal{L}_{\rm int} \supset m_e \overline{\hat{e}} \hat{e} \left(-\frac{1}{3} \frac{\delta \mathcal{V}}{\langle \mathcal{V} \rangle}\right) \,.
\end{equation}
%
...
\mathcal{L}_{\rm int} \supset \frac{\sigma_1}{\langle \mathcal{V} \rangle^2} \frac{\varphi_2}{M_{\rm p}} \, m_e \overline{\hat{e}} \hat{e} \,,
\end{equation}
where
%
\begin{equation}
...
\bel{nmatrix}
    ...
    \begin{array}{ccc}
    \cos(\theta+\hat{\alpha}) && -\sin(\theta + \hat{\alpha}) \\
    \sin(\theta-\hat{\alpha}) && \cos(\theta - \hat{\alpha}) \\
    \end{array}%
    \right) \,.
\end{equation}
%
where the angles $\theta$ and $\hat{\alpha}$ are 
%
\begin{equation}
    \sin 2\hat{\alpha} = \lambda \qquad \hbox{and} \qquad
    \tan 2\theta = \frac{2\mu^2 - (m_{\scriptscriptstyle A}^2 + \mathbf^2)
    \lambda}{(m_{\scriptscriptstyle A}^2 - \mathbf^2) \sqrt{1-\lambda^2}} \,.
\end{equation}
%
The mass eigenvalues $M_{\pm}$ are
%
\begin{equation}
    M^2_{\pm} = \frac{m_{\scriptscriptstyle A}^2 + \mathbf^2 - 2 \lambda \mu^2
...
%
\begin{equation}
    \Delta^2 = (m_{\scriptscriptstyle A}^2 - \mathbf^2)^2 + 4\mu^4 - 4\lambda \mu^2 (m_{\scriptscriptstyle A}^2
    + \mathbf^2) + 4\lambda^2 m_{\scriptscriptstyle A}^2 \mathbf^2 \,.
\end{equation}
%
...
\item If $m_{\scriptscriptstyle A}^2 = \mu^2 = 0$, then one of the gauge fields  (corresponding to the photon) is
massless. The other gauge field has mass $M_+^2 =
\mathbf^2/(1-\lambda^2)$ . Also $\sin 2\theta = \sin 2\hat{\alpha} = \lambda$, as a result of this only the massive (hidden) vector acquires a coupling to both currents
%
\begin{equation} \label{eq:masslessmassivemixing}
...
%
\begin{equation}\label{eq:masslessBoson}
    {\cal L}_{\rm int} = \frac{\cos\hat{\alpha}}{\sqrt{1-\lambda^2}}
    \left( j_1^\mu {\cal A}^-_\mu
    + j_2^\mu {\cal A}^+_\mu \right)
    - \frac{\sin\hat{\alpha}}{\sqrt{1-\lambda^2}}
    \left( j_1^\mu {\cal A}^+_\mu
    + j_2^\mu {\cal A}^-_\mu \right) \,,
\end{equation}
%
with $\sin^2\hat{\alpha} = \frac12 \left[ 1 - \sqrt{1-\lambda^2}
\right]$ and $\cos^2\hat{\alpha} = \frac12 \left[ 1 + \sqrt{1 -
\lambda^2} \right]$ .
%
Note that, in this case the coupling of the visible sector photon ${\cal{A}}_{\mu}^{1}$  to $j^{\mu}_1$ {\it depends} on the 
mixing parameter. Thus, a change in the mixing parameter caused due to a rolling quintessence scalar leads to time variation in the fine structure constant. For small $\lambda$, $\cos \hat{\alpha} \sim 1 - {\lambda^2 \over 8}$. Thus we have $ { \delta \alpha / \alpha} =  {\delta(\lambda^2) \over 4}$. Making use of the expression for for strength of the mixing parameter in Eq. \pref{strmix} 
%
$$
...
\begin{equation}
\label{singk}
   K \supset { \tau_q^2 \over \mathcal{V} }.
\end{equation}
%
...
\begin{equation}
\label{orbmix}
   K \supset {\tau_q \tau_{\rm sm}  \over \mathcal{V}^{p}} \ .
\end{equation}
%
