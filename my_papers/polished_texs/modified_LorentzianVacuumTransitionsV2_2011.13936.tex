...
In the corresponding multi-dimensional quantum mechanical case the expression for $S_{1}$ is given by the VanVleck determinant~\cite{VanVleck:1928zz} (see also~\cite{Brown:1971zzc}). To see the connection
...
in~\cite{Parke:1982pm}.}. This gives $\frac{d\tau}{d\rho}=\pm1/\sqrt{1-V_{B,A}\rho^{2}}$
in the first and third terms\footnote{In~\cite{Parke:1982pm} only the positive sign is kept here.}
...
\end{equation}
 Here $a(0)=0$ is a coordinate singularity. It is clear from Eqs. \eqref{eqn:fieldinflation} that $\dot a$ could not be chosen to vanish at $t=0$  for positive potentials. We can see that initially the dynamics is dominated by the curvature term $1/a^2$. The friction ($3H$) diverges at $t\textrightarrow 0$, and then the initial $\dot\phi$ does not increase by much. 
...
\end{equation}
This foliation does not describe the full dS but only the causal patch of an observer at the centre of the hyperboloid. The region covered is  depicted in red in Fig.~\ref{fig:deSitter_hyperbolic_slicing}. Note that  the analytical  continuation $\chi\textrightarrow i \chi$ leads to
\begin{equation}
ds_E^2=d\xi^2+\Lambda^2\sin^2(\xi/\Lambda)(d\chi^2+\cos^2\chi^2 d\Omega_2^2) \,,
\label{metric:euclidean}
\end{equation}
which is the euclidean metric of a 4 sphere. This  continuation was equivalent to make $X_0\textrightarrow i X_0$.
\begin{figure}[h!] 
\begin{center} 
...
To get  the Euclidean continuation we need to do the transformations, $\chi\textrightarrow i \chi$  and $\xi\textrightarrow i \xi$, which lead to Eq.~\eqref{metric:euclidean}. finally note that  this transformation are equivalent to $X_0\textrightarrow iX_0$ .
...
\end{equation}
and  writing $\psi\textrightarrow \pi/2+i\sigma$ we get the $O(3,1)$ invariant metric,
\begin{equation}
...
\item This space is not geodesically complete, because for timelike geodesics it is possible to go past the lightlike hypersurfaces. To complete the description we can make a further analytical continuation $\sigma\textrightarrow i \pi/2+\chi$ and a rotation $\xi\textrightarrow it$ which leads to,
\begin{equation}
ds^2=-dt^2+\rho^2(-i \xi)(d\chi^2+\sinh^2\chi d\Omega_2^2).
\end{equation}
This region describes an  FRW open space, and covers the blue patch in Fig.~\ref{fig:deSitter_hyperbolic_slicing}. Note that  the analytical continuation  $\sigma\textrightarrow -i \pi/2+\chi$  describes the upper right region of the diagram.    Constant time hypersurfaces end on  $\mathscr{I}_+$ at $\xi=\pi/2$
