...
\end{equation}
where $\alpha = g_2^2/(4\pi)$. Given that $\langle \sigma_{\rm ann} v \rangle \simeq \alpha^2/m_\chi^2$, weakly interacting particles with masses around the weak scale $m_\chi\sim m_{\rm weak}\sim\mathcal{O}(100)$ GeV naturally give rise to the observed DM relic density. This fact is very well known in the literature under the name of ‘WIMP miracle’ and it suggests that new degrees of freedom at the weak scale are natural DM candidates.
...
From Figs. \ref{Fig2} - \ref{Fig4} we can extract another interesting information about the sparticle spectrum. Given that in the sequestered LVS scenario $M_{1/2}$ is universal at the GUT scale, binos, winos and gluinos have different masses. If the DM relic density is saturated by higgsinos with $m_\chi\simeq 300$ GeV, we have $m_{\tilde{B}}\simeq 1.9$ TeV, $m_{\tilde{W}}\simeq 3.8$ TeV and $m_{\tilde{g}}\simeq 10.2$ TeV. Another interesting situation would be the case with $m_\chi\simeq 600$ GeV since it is in the region close to the LZ detection reach. In this case the spectrum of gauginos would be $m_{\tilde{B}}\simeq 6$ TeV, $m_{\tilde{W}}\simeq 12.3$ TeV and $m_{\tilde{g}}\simeq 33.3$ TeV. In both cases sfermion masses are at least on the multi-TeV range (typically $\mathcal{O}(10)$ TeV) or heavier (their detailed spectrum depends on whether the SUSY model is split-like or not).
...
\begin{enumerate}
\item In non-thermal cosmologies with an extra period of matter domination which ends via reheating with temperatures of $\mathcal{O}(1 - 10)$ GeV (above BBN), light higgsinos with masses as low as a few hundred GeV can correctly saturate the DM content measured by the Planck satellite. 
