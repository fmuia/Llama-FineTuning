...
\newcommand{{\bf  z}}{{\bf  z}}
\newcommand{{\bf  b}}{{\bf  b}}
\newcommand{\tau_{\star}}{\tau_{\star}}
\newcommand{t_{\star}}{t_{\star}}
\newcommand{\bar\phi}{\bar\phi}
\def{ d{{ d}}
\def{\rm d{{\rm d}}
\def{\rm Tr{{\rm Tr}}
\def{\tau_{\rm rec{{\tau_{\rm rec}}}
\def{\cal G{{\cal G}}
\def\Gamma^{\dagger{\Gamma^{\dagger}}
\def{\cal C{{\cal C}}
\def{\boldsymbol 0{{\boldsymbol 0}}
\def{\boldsymbol{e{{\boldsymbol e}}
\def{\vec{k{{\vec{k}}}
\def{\boldsymbol{r{{\boldsymbol{r}}}
\def{\vec{m{{\vec{m}}}
\def{\boldsymbol{q{{\boldsymbol{q}}}
\def{\boldsymbol{l{{\boldsymbol{l}}}
...
\begin{equation}
S=\int d^4 x\sqrt{-g} \left[M_\mathrm{Pl}^2 R-\frac{1}{2}\partial_\mu\phi\partial^\mu\phi-V(\phi)\right] \,.
\end{equation}
...
with 
\begin{eqnarray}
S_{g} & = &M_\mathrm{Pl}^2\int dt d^3 x\left(-N^{-1}3a\dot{a}^{2}+3aN\right) \,,\label{eq:Sg}\\
...
\begin{align}
H^2\equiv\left(\frac{\dot a}{a}\right)^2&=\frac{1}{3M_\mathrm{Pl}^2}\left(\frac{1}{2}\dot\phi^2+V(\phi)\right)-\frac{1}{a^2} \,,\\
...
so inflation lasts as long as $-\dot H/H^2 <1$. This can be phrased in terms of the scalar field using the equations of motion, 
 \begin{align}
 \label{eq:Epsilon}
\frac{\vert\dot H\vert}{H^2}-\frac{1}{a^2H^2}=\frac{1}{M_\mathrm{Pl}^2}\frac{\dot\phi^2}{2H^2} \equiv \epsilon \,.
 \end{align}
...
\end{equation}
where $\gamma$ is the metric over the unit 3-sphere and $\mathcal{L}_{\mathrm{int}}(N,\varphi)$ parametrises higher order interactions. As it is shown in Appendix~\ref{sec:ADMPert}, in flat gauge  the dependence on $M_\mathrm{Pl}^2$ does not appear explicitly. Instead, when the action is  in the comoving gauge the factor of $M_\mathrm{Pl}^2$ reappears (see Eq.~\eqref{eq:actioncomovinggaguge}).  The action for the perturbations in Eq.~\eqref{eq:PertAction} is then   proportional to $M_\mathrm{Pl}^2\dot\phi$, which implies that the  perturbation field  only makes sense when the slow-roll parameters are small but non vanishing.

 In order to study the dynamics of this system we will first need the   conjugate momenta,
\begin{align}
&\pi_{N}=0\,, \qquad \pi_{a}=-M_\mathrm{Pl}^2 N^{-1}6a\dot{a} \,,\label{eq:momenta}\nonumber\\
...
\begin{align}
{\cal H}_0&=N\left(-\frac{1}{M_\mathrm{Pl}^2}\frac{\pi_{a}^{2}}{12a}+\frac{\pi_{\phi}^{2}}{2a^{3}}-3aM_\mathrm{Pl}^2+a^{3}V(\phi)\right) \,,\label{eq:calH}\nonumber\\
...
\begin{align}
\epsilon= \frac{1}{M_\mathrm{Pl}^2}\frac{\dot\phi^2}{2H^2}=3 a^2M_\mathrm{Pl}^2\frac{\pi_\phi^2}{\pi_a^2}\ll 1 \,,
\end{align}
...
$\psi=\frac{1}{\sqrt{dS_0/da}}\exp\left(iM_\mathrm{Pl}^2 S_0\right)$ corresponds to the  minisuperspace semi-classical wave-function (with pre-factor) and $\chi(a,\varphi)$ corresponds to the perturbations in that background.

$S_0$ is the solution of
\begin{equation}
-\frac{M_\mathrm{Pl}^2}{12a}\left(\frac{\delta S_0}{\delta a}\right)^2 +a^3 V(\phi)-3aM_\mathrm{Pl}^2=0 \,,
\end{equation} 
which is the Hamilton-Jacobi equation for minisuperspace. 
Here the function $f(a,\phi)=a^3 V(\phi)-3aM_\mathrm{Pl}^2$ can be considered as an effective scalar potential for $a$ and $\phi$. For $V>0$ this has a barrier in the $a$ direction for $a<\sqrt{3/V}M_\mathrm{Pl}$ (remembering that $a$ has the wrong sign of the kinetic term).
In the under the barrier region $a^3V<3a M_\mathrm{Pl}^2$ we get \footnote{A factor $2\pi^2$ corresponding to the volume of the unit three sphere has been inserted in the formulae for $S_0$.},
\begin{equation}
\label{eq:S0underthebarrier}
S_0=\pm 12\pi^2i \int da a \sqrt{1-\frac{a^2 V}{3M_\mathrm{Pl}^2}}=\mp i \frac{12\pi^2M_\mathrm{Pl}^2}{V}\left(1-\frac{a^2V}{3M_\mathrm{Pl}^2}\right)^{3/2} \,.
\end{equation}
In the over the barrier region $a^3V>3aM_\mathrm{Pl}^2$ we have instead
\begin{equation}
\label{eq:S0overthebarrier}
S_0=\pm 12\pi^2 \int da a \sqrt{\frac{a^2 V}{3M_\mathrm{Pl}^2}-1}=\pm  \frac{12\pi^2M_\mathrm{Pl}^2}{V}\left(\frac{a^2 V}{3M_\mathrm{Pl}^2}-1\right)^{3/2} \,.
\end{equation}
...
\begin{equation}
\pi_a=M_\mathrm{Pl}^2\frac{\partial S_0}{\partial a}=-12\pi^2M_\mathrm{Pl}^2N^{-1}a\dot a \,.\label{eq:pi}
\end{equation} 
At $\mathcal{O}(M_\mathrm{Pl}^0)$ we have a Schr\"odinger equation for $\chi$
\begin{equation}
...
\begin{align}
a&= \sqrt{\frac{3M_\mathrm{Pl}^2}{V}}\sin{\left(\sqrt{\frac{V}{3}}\frac{\tau}{M_\mathrm{Pl}}\right)},\qquad\mathrm{for}\ a^3V<3aM_\mathrm{Pl}^2,~0<\sqrt{\frac{V}{3}}\frac{\tau}{M_\mathrm{Pl}}<\pi/2 \,,\label{eq:a_underthebarrier}\\
a&= \sqrt{\frac{3M_\mathrm{Pl}^2}{V}}\cosh{\left(\sqrt{\frac{V}{3}}\frac{t}{M_\mathrm{Pl}}\right)},\qquad\mathrm{for}\ a^3V>3aM_\mathrm{Pl}^2,\,~t>0 \,.
...
\end{align}
where $t$ is cosmic time and we defined $M_\mathrm{Pl}^{2}H^2=V/3$.
...
\begin{align}
\pi_\varphi\textrightarrow-i\frac{\delta}{\delta\varphi} \,,
\end{align}
...
\end{equation}
This is in fact the ‘radial’ equation coming from the 4D Laplace equation in the metric of Eq.~\eqref{eq:minimetric} in Euclidean conformal coordinates in the gauge $N=1$, i.e. $ds^2=a^2(T)(dT^2+d\Omega_3^2)$. To solve this equation we take  the scale factor  given by the de Sitter solution  $a(T)=H^{-1}\sech(T)$. Imposing the solutions to be regular at $a=0$ which in these coordinates correspond to $T\textrightarrow-\infty$, we get that the mode function is,\footnote{In the case the field has a mass $m_\phi$ the equation  changes to $\partial_T\left(a^2 \partial_T f\right)=a^2(p(p+2))f+ a^4 m_\phi^2 f$ the solutions are
\begin{align}
...
\end{equation}
We would like to compare this result to the flat space mode function $f_k\sim\frac{e^{-ik\eta}}{k^{3/2}}(i+k\eta)$ where $\eta$ is conformal time and $a=-1/(H\eta)$. The analogy  is not exact but if we look at the limit $\eta\textrightarrow\pi/2$ then both scale similarly at large $l$: $f_k \sim k^{-3/2}$ and $f_p \sim p^{-3/2}$. Moreover when $\eta$ is close to zero the closed mode function scales similarly as the Minkowski vacuum. For future reference we also write down the expression for the functions $c_p = - a^2 \partial_\eta f_p/f_p$:
\begin{equation}
\label{eq:clL}
...
From the wave-function we can compute the two point function to be,
 ...
 \begin{equation}
\boxed{ \langle \zeta_p\rangle^2=\frac{H^4}{\dot\phi^2}\frac{1}{2p(p+1)(p+2)}=\frac{H^4}{2M_\mathrm{Pl}^2 (\dot H-\frac{1}{a^2})}\frac{1}{2p(p+1)(p+2)}}
...
\end{align}
Note that  $\eta<0$ corresponds to the Euclidean region  which we get by tha continuation $\eta\textrightarrow i T$. In this region the mode functions are given by Eq.~\eqref{eq:modefunctionEuclid} and the scale factor by Eq.~ \eqref{eq:EuclideanScaleFactor}. In the end, since the mode function has a factor of $\exp(i(p(p+1)T))$, the integrand goes to zero  in the limit $T\textrightarrow\infty$. This is similar to the contour rotation done in the in-in computations on a flat universe~\cite{Maldacena:2002vr}, where the integrals vanish at $a=0$. 
...
\begin{align}
S_3=\frac{1}{M_\mathrm{Pl}}\int dt d^3 x \sqrt{\gamma} \left(\frac{a}{\sqrt{2\epsilon}}\varphi^3+a^5\sqrt{2\epsilon} H\dot\varphi^2 \partial^{-2}\dot\varphi +\frac{\delta \mathcal{L}_2}{\delta\varphi}f(\varphi)\right) \,,
...
\begin{align}
S_3=\frac{1}{M_\mathrm{Pl}}\int d^4x \left(\frac{a}{\sqrt{2\epsilon}}\varphi_c^3+a^5\sqrt{2\epsilon} H\dot\varphi_c^2(\delta)^{-2}\dot\varphi_c \right) \,.
\end{align}
...
\begin{align}
\mathcal{H}_{\rm{int}}=-\frac{1}{M_\mathrm{Pl}}\int  d^3 x\left(\frac{a^2}{\sqrt{2\epsilon}}\varphi_c^3+\frac{\sqrt{2\epsilon}H}{a^3}\pi_\varphi^2\partial^{-2}\pi_\varphi\right) \,.
\label{eq:IntHamPNG}
\end{align}
...
\begin{align}
\left(\partial_\eta-\frac{1}{a^2}\sum_i c_{p_i}\right)c^{(3)}_{p_1,p_2,p_3}=\frac{1}{M_\mathrm{Pl}}\frac{a^2}{\sqrt{2\epsilon}}+\frac{1}{M_\mathrm{Pl}}\frac{\sqrt{2\epsilon}H}{a^3}\sum _{\rm{perm}}c_{p_1}c_{p_2}\partial^{-2}c_{p_3} \,.
\end{align}
...
\begin{align}
\partial_{\eta}c^I_{p_1,p_2,p_3}=\frac{1}{M_\mathrm{Pl}}\frac{a^2}{\sqrt{2\epsilon}}\prod_{i}f_{p_i}+\frac{1}{M_\mathrm{Pl}}\frac{\sqrt{2\epsilon} H}{a^3}\prod_{i}f_{p_i}\sum _{\rm{perm}}c_{p_1}c_{p_2}\partial^{-2}c_{p_3} \,,
\label{eq:IntegraPNG}
\end{align}
...
\begin{align}
c^{(b)}_{p_1,p_2,p_3}&=\frac{1}{M_\mathrm{Pl}}\frac{\sqrt{2\epsilon_*} H_*}{\prod_{i}f_{p_i}}\int_{-\infty}^{\pi/2} d\eta \, a^{-3}\prod_{i}f_{p_i}\sum_{\rm{perm}}c_{p_1}c_{p_2}\frac{1}{-p_3(p_3+2)}c_{p_3}\,,\nonumber
\end{align}
...
\begin{equation}
c^{(b)}_{p_1,p_2,p_3} = i\frac{1}{M_\mathrm{Pl}}\frac{\sqrt{2\epsilon_*}}{H_*^2} \,  \times \frac{\sum_{i>j}p_i(p_i+2)p_j(p_j+2)}{p_T+3} \,,
\end{equation}
where the asterisk denote the value at horizon crossing. This is equivalent to the flat space case, where the contour is rotated in the past, $\eta\textrightarrow-(1+i\epsilon)\infty$ to select the interacting vacuum. Notice that the cubic coefficient has an overall $i$, which implies that there it is a small contribution in the wave function since it appears schematically as $\chi\sim\exp(- l^3 \varphi^3)$. Finally, using Eq.~\eqref{eq:3PointCF}, the three point function is given by 
\begin{equation}
\boxed{\langle\varphi_{c, p_1}\varphi_{c, p_2}\varphi_{c, p_3}\rangle'=\frac{\sqrt{2\epsilon_*}}{4}\frac{H_*^4}{M_\mathrm{Pl}}\prod_{i}\frac{1}{p_i(p_i+1)(p_i+2)}\times\frac{\sum_{i>j}p_i(p_i+1)p_j(p_j+1)}{p_T+3}} \,.
\end{equation}
...
\begin{align}
\label{eq:3pfFR12}
\langle  \varphi_{p_1}\varphi_{p_2}\varphi_{p_3}\rangle_{\rm{fr}}'=\frac{\sqrt{2\epsilon_*}}{32}\frac{H_*^4}{M_\mathrm{Pl}}\prod_{i}\frac{1}{p_i(p_i+1)(p_i+2)}\times\left(\sum_i p_i(p_i+1)(p_i+2)+\sum_{i\neq j}(p_i+1)p_j(p_j+2)\right)
\end{align}
...
\begin{align}
\label{eq:3pfFR13}
\langle \varphi_{p_1}\varphi_{p_2}\varphi_{p_3}\rangle_{\rm{fr}}'=\frac{3\sqrt{2\epsilon}}{32}\frac{H_*^4}{M_\mathrm{Pl}}\prod_{i}\frac{p_T+3}{p_i(p_i+1)(p_i+2)} \,.
\end{align}
Summing all the terms, the three point function is given by,
\begin{small}
\begin{align}
\langle \varphi_{p_1}\varphi_{p_2}\varphi_{p_3}\rangle'&=\frac{H_*^4}{M_\mathrm{Pl}}\frac{\sqrt{2\epsilon_*}}{32\prod_{i}p_i(p_i+1)(p_i+2)}\\
...
\begin{align}
\zeta (t,{\vec{x)= \sum_{n=1}^{\infty}\frac{1}{n!}\left(\frac{\partial ^nN(t,t^*)}{\partial\phi_*^n}\right)(\varphi_*(t_*,x))^n \,,
\end{align}
...
\begin{align}
\frac{\delta N}{\delta \phi_*}=-\frac{H_*}{\dot\phi_*} \,,\qquad \frac{\delta^2 N}{\delta\phi_*^2}=\frac{H_*\ddot \phi_*}{\dot\phi^3_*}+\frac{1}{2M_\mathrm{Pl}^2}-\frac{1}{\dot\phi_*^2 a^2} \,,
\end{align} with all of this we finally have,
\begin{small}
\begin{align}
\label{eq:closedBispectrum2V}
\langle \zeta_{p_1}\zeta_{p_2}\zeta_{p_3}\rangle'&=\frac{H_*^4}{M_\mathrm{Pl}^4}\frac{1}{2\epsilon_*\times 32}\frac{1}{\prod_{i}p_i(p_i+1)(p_i+2)}\\
&\times\left(\sum_i p_i(p_i+1)(p_i+2)+\sum_{i\neq j}(p_i+1)p_j(p_j+2)+8\frac{\sum_{i>j}p_i(p_i+2)p_j(p_j+2)}{p_T+3}+3(p_T+3)\right) \nonumber\\
&+\frac{H_*^4}{M_\mathrm{Pl}^4}\frac{1}{4\epsilon}\left(\frac{1}{2\epsilon}\left(\frac{\ddot \phi_*}{\dot\phi_*H_*}+\frac{1}{a_*^2 H_*^2}\right)-\frac{1}{2}\right)\frac{1}{\prod_i 2p_i(p_i+1)(p_i+2)}\sum_i p_i(p_i+1)(p_i+2) \,,\nonumber
\end{align}
\end{small}
...
\begin{align}
\langle\zeta_{\k_1}\zeta_{\k_2}\zeta_{\k_3}\rangle'=\frac{H^4_*}{M_\mathrm{Pl}^4}\frac{1}{4\epsilon^2}\frac{1}{\prod_i 2k_i^3}\left[\epsilon\left(\sum_i k_i^3+\sum_{i\neq j}k_ik_j^2+\frac{8}{k_T}\sum_{i>j}k_i^2 k_j^2\right)+\frac{2\ddot\phi_*}{\dot\phi_* H_*}\sum_i k_i^3\right] \,,
\label{eq:PNG_flatU}
\end{align}
...
\begin{align}
c^{(a)}_{p_1,p_2,p_3}&=\frac{1}{\sqrt{2\epsilonM_\mathrm{Pl}} \prod_i f_{p_i}}\int^{\pi/2}_{-\infty} d\eta \, a^2\prod_i f_{p_i}\label{eq:ca}\\
&\simeq-\frac{i}{\sqrt{2\epsilon}M_\mathrm{Pl} H^2}\left(-(p_T+1)+\frac{p_1p_2 p_3}{(p_T+2)(p_T+4)} + \frac{\sum_{i>j} p_i p_j}{(p_T+2)}\right) - \frac{1}{\sqrt{2\epsilon} H^2}\frac{1}{\eta-\pi/2} \,, \nonumber
\end{align}
...
\begin{align}
\langle \varphi_{p_1}\varphi_{p_2}\varphi_{p_3}\rangle'=\frac{H_*^4}{4\sqrt{2\epsilon_*}M_\mathrm{Pl}}\frac{1}{\prod_{i}p_i(p_i+2)(p_i+2)}\left(-(p_T+1)+\frac{p_1p_2p_3}{(p_T+2)(p_T+4)}+\frac{\sum_{i>j}p_i p_j}{(p_T+2)}\right) \,.
\label{PNG}
\end{align}
Writing this expression in the comoving gauge we find, 
\begin{align}
\langle \zeta_{p_1}\zeta_{p_2}\zeta_{p_3}\rangle'=\frac{H_*^4}{M_\mathrm{Pl}^2}\frac{1}{4(2\epsilon)^2}\frac{1}{\prod_{i}p_i(p_i+2)(p_i+2)}\left(-(p_T+1)+\frac{p_1 p_2 p_3}{(p_T+2)(p_T+4)}+\frac{\sum_{i>j}p_i p_j}{(p_T+2)}\right) +\dots\,.
\label{eq:zeta3prime}
\end{align}
...
\begin{align}
\tilde{\Delta}_\zeta^2\equiv \frac{H_*^4}{M_\mathrm{Pl}^2}\frac{1}{4\epsilon}
\end{align}
...
\begin{align}
S=\int &d^4 x \sqrt{-g}\left[\frac{1}{2}M_\mathrm{Pl}^2R+M_\mathrm{Pl}^2\left(\dot H-\frac{k}{a^2}\right)\cdot g^{00}-M_\mathrm{Pl}^2\left(3H^2+\dot H+2\frac{k^2}{a^2}\right)\right.\nonumber\\
&\left.+\frac{1}{2!}M(t)(g^{00}+1)^2+\frac{1}{3!}c_3(t)M(t)^4(g^{00}+1)^3+...\right] \,.
\end{align}
The Goldstone boson $\pi$ associated to broken time translations is introduced through the St\"uckelberg trick, $t\textrightarrow t+\pi$. Up to cubic order, and  neglecting the mixing with the graviton, this leads to the following action
\begin{align}
S=\int d^4 x\sqrt{-g} \, \frac{\epsilon H^2M_\mathrm{Pl}^2}{c_s^2}\left[\left(\dot\pi^2-c_s^2\frac{(\nabla\pi^2)}{a^2}\right)+\alpha\dot\pi\frac{(\nabla\pi)^2}{a^2}+\beta\dot\pi^3\right] \,,
\label{eq:EFTOI}
\end{align}
where $g$ is the minisuperspace metric determinant and the speed of sound $c_s$ is defined as,
\begin{align}
c_s^{-2}=1-\frac{2M^4}{M_\mathrm{Pl}^2(\dot H-ka^{-2})} \,,
\label{eq:cs}
\end{align}
and,
\begin{align}
...
In order to write the Hamiltonian it is convenient to define the gravitational coupling scale $f_\pi^4\equiv 2M_\mathrm{Pl}^2 \vert\dot H-1/a^2\vert c_s^{-2}$  . In terms of this the  conjugate momenta is,
\begin{align}
...
$P_\pi\textrightarrow i\frac{\delta}{\delta_\pi}$. Doing so we obtain the Hamilton-Jacobi equation
\begin{align}
...
\label{eq:cl_EFT}
\end{align}
We are interested in the two point function of $\zeta$ at $t\textrightarrow\pi/2$ which is  
\begin{align}
\langle\zeta^2\rangle&=H^2\langle\pi^2\rangle=\frac{H^4}{2c_s^2 f_\pi^4}\frac{1}{p(p+2)\sqrt{1+c_s^2 p (p+2)}}\nonumber\\
&=
\frac{H^4}{4 M_\mathrm{Pl}^2(\dot H-1/a^2)}\frac{1}{p(p+2)\sqrt{1+c_s^2 p(p+2)}} \,.
\end{align}
...
\begin{align}
\langle\zeta^2\rangle= \frac{H^4}{4M_\mathrm{Pl}^2(\dot H-1/a^2)c_s}\frac{1}{p^3}\left(1-\frac{3}{p}+\frac{15 c_s^2-1}{2c_s^2}\frac{1}{p^2}+\mathcal{O}\left(p^{-3}\right)\right) \,.
...
\end{align}
where we have used that in the $\eta\leq 0$ the mode function is given by its analytic continuation $\eta\textrightarrow-iT$ such that the integral converges  at $a=0$. The cubic coefficient has an additional dependence on the speed. At leading order in the small parameter $1-c_s \ll 1$ we find,
\begin{equation}
...
\begin{align}
\langle\zeta_{p_1}\zeta_{p_2}\zeta_{p_3}\rangle&=\frac{\beta c_s^4 H^4}{8 M_\mathrm{Pl}^4\epsilon^2}\prod_{i=1}^3\frac{1}{\sqrt{1+c_s^2 p_i(p_i+2)}}\\
...
\end{align}
Notice that $\dot\phi^2= M_\mathrm{Pl}^2(\dot H-a^{-2})$, hence in the action in comoving gauge reintroduces the dependence on $M_\mathrm{Pl}^2$.
