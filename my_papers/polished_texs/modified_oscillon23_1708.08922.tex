...
\begin{equation}
\mathcal{V} = \alpha \left(\tau_{\rm b}^{3/2} - \sum_{i=2}^N \alpha_{i} \tau_{{\rm s}, i}^{3/2}\right)\,,
\end{equation}
...
\begin{eqnarray}
K/M^2_{\rm Pl} &=& - 2 \log\left(\mathcal{V} + \frac{\xi s^{3/2}}{2}\right),\\
...
\frac{V_{\mathcal{O}\left(\mathcal{V}^{-3}\right)}}{M^4_{\rm Pl}} = \frac{g_s}{8 \pi} \left[\sum_{i=1}^N \left(\frac{8}{3} \left(\frac{a_i A_i}{\alpha_i}\right)^2 \sqrt{\tau_{s, i}} \,\frac{e^{-2 a_i \tau_{s, i}}}{\mathcal{V}} -  4 W_0  \,a_i A_i \tau_{s, i} \,\frac{e^{-a_i \tau_{s, i}}}{\mathcal{V}^2} \right) + \frac{3 \hat{\xi} |W_0|^2}{4 \mathcal{V}^3}\right] + V_{\text{dS}} \,,
\end{equation}
where $\hat{\xi}=\xi/g_s^{3/2},$ $V_{\text{dS}}=D/\mathcal{V}^\gamma$ ($1\leq\gamma< 3$) is an additional contribution from localised sources where $D$ is fine-tuned to uplift the potential to an approximate Minkowski minimum. In addition, there is an overall rescaling of the entire potential arising from the VEV of the complex structure \Kahler potential $e^{K_{\rm cs}}$, which, unless otherwise stated, we set to unity. The minimum with respect to the small moduli is given by
\begin{equation}
a_i A_i e^{-a_i\tau_i}=\frac{3\alpha\alpha_i}{\mathcal{V}}\frac{(1-a_i\tau_i)}{(1-4a_i \tau_i)}\sqrt{\tau_i}~.
\end{equation}
In the limit $\mathcal{V}\textrightarrow \infty$ this gives
\begin{equation}
a_i \tau_i\approx \log{(\mathcal{V})}~.
\end{equation}
...
\begin{equation}
\frac{\partial V}{\partial \mathcal{V}}=0~,\qquad V=0~.
\end{equation}
This leads to an exponentially large value of the volume in the minimum (in string units)
\begin{equation}
\log{\mathcal{V}}\approx \frac{\hat{\xi}}{2}\left(\sum\frac{\alpha_i \alpha}{a_i^{3/2}}\right)^{-1}~.
\end{equation}
The value of $D$ can be determined numerically. The masses for the volume and small moduli in the minimum are given by
\begin{eqnarray}
m_{\tau_i}^2&\simeq & M^2_{\rm Pl}\frac{W_0^2 (\log{\mathcal{V}})^2}{\mathcal{V}^2}~,\\
m_{\mathcal{V}}^2&\simeq & M^2_{\rm Pl}\frac{W_0^2 }{\mathcal{V}^{3}\log{\mathcal{V}}}~.
\end{eqnarray}
