...
\newcommand{$N_{\rm eff}$ }{$N_{\rm eff}$ }
\newcommand{$\Lambda$CDM}{$\Lambda$CDM}
...
\def\partial{\partial}
\def\phi_*{\phi_*}
\deff_\kappa{f_\kappa}
\deft_b{t_b}
\def{\scriptscriptstyle \rm bulk{\beta}
\def\mathcal{V}{{\cal{V}}}
\def{\boldsymbol{r{r_b}
\def\hat{\varphi{\hat{\varphi}}
\def\vec{p{\varphi}
\defM_\mathrm{Pl}{M_{\rm pl}}
\def\cal{V{\cal{V}_{\rm in}}
\def\chi_b{\chi_b}
\def{\cal G{\Gamma}
\def\gtrsim{\gtrsim}
\def{B_\th{{B_\th}}
...
\def\noindent{\noindent}
\def~\mbox{\raisebox{-.6ex{~\mbox{\raisebox{-.6ex}{$\stackrel{<}{\sim}$}}~}
\def~\mbox{\raisebox{-.6ex{~\mbox{\raisebox{-.6ex}{$\stackrel{>}{\sim}$}}~}
\def\ \raise.3ex\hbox{$<$\kern-.75em\lower1ex\hbox{$\sim${\ \raise.3ex\hbox{$<$\kern-.75em\lower1ex\hbox{$\sim$}}\ }
\def\ \raise.3ex\hbox{$>$\kern-.75em\lower1ex\hbox{$\sim${\ \raise.3ex\hbox{$>$\kern-.75em\lower1ex\hbox{$\sim$}}\ }
\def\ \raise.5ex\hbox{$>${\ \raise.5ex\hbox{$>$}\kern-.8em\lower.5ex\hbox{$<$}\ }
...
\def\text{mix{\text{mix}}
\def\text{tot{\text{tot}}
\def\mathcal{H{\mathcal{H}_{\text{inf}}
\def\nonumber{\nonumber}
\defa_i \tau_i{a_i \tau_i}
\def\phantom{a{\phantom{a}}
\def\pref#1{(\ref{#1})}
\def\frac12{\frac12}
\def\partial{\partial}
...
\def\oplus{\oplus}
\def{{\cal{V{{{\cal{V}}}}
\def{\hat{\cal{V{{\hat{\cal{V}}}}
\defM_\mathrm{Pl}{M_{\rm pl}}
\def\vec{p{\vec{p}}
\def\varphi{\varphi}
\defm_{\varphi{m_{\varphi}}
\defm_{\varphi{m_{\varphi}}
\def\hat{m{\hat{m}}
%\def\hat{s{\hat{s}}
...
\newcommand{\Delta N_{\rm eff}}{\Delta N_{\rm eff}}
...
\defH\,{\sc ii{H\,{\sc ii}~}
\def{\vec{k{{\bm k}}
\def\bar{x{\bar{x}_{{\rm H\,\textsc{\scriptsize i}}}}
\def{\vec{x{{\bf \textit{x}}}
...
If the background component is an oscillating modulus with mass $m_b$ and decay rate $\Gamma_b = m_b^3/M_\mathrm{Pl}^2$, then Eq.~\eqref{d2} would imply
\begin{equation}
m_{b, \rm min} \lesssim m_b \lesssim m_{b, \rm min} \exp\left(\frac{3}{4} (N_{\rm BBN} - N_{\rm NL})\right) \,,
\end{equation}
where $m_{b, \rm min} = \left(H_{\rm BBN} M_\mathrm{Pl}^2\right)^{1/3} \simeq 20 \, \text{TeV}$.
...
\begin{equation}
\rho({N_{\rm NL}}) = 3 H_{\rm NL}^2 M_\mathrm{Pl}^2 = 3 \left(\frac{H_{\rm NL}}{H_{\rm in}}\right)^2 H_{\rm in}^2 M_\mathrm{Pl}^2 \,.
\end{equation}
...
\begin{align}
\label{eq:OmegaGWk}
\Omega_{{\rm GW}, p}(k_p) &\sim \frac{64 \pi^2}{3 M_\mathrm{Pl}^4 H_{\rm NL}^4} \frac{\rho_{\phi}^2(N_{\rm NL})}{(k_p/(a_{\rm NL} H_{\rm NL}))^2} \frac{\alpha^2}{\lambda} \nonumber \\
